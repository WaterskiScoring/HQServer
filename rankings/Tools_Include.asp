...
'			response.write("<br>Line 432 Tools_Include - sShowGR(EvtNo)=true = ")
'			response.write(sShowGR(EvtNo)=true)
'	END IF
'END IF	
...
SELECT CASE sTSptsGrpID
  	CASE "AWS"
		...
		SELECT CASE TRIM(EventSelected) 
		   ...
		   CASE "J"
					IF NOT (sTHJClassR OR sTHJClassL OR JClassR>0 OR JClassL>0) THEN sSQL = sSQL + " AND lower(left(DT.div,1)) <> 'i' AND lower(DT.div)<>'b1' AND lower(DT.div)<>'g1'"
