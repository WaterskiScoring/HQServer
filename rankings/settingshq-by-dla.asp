...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-27835299-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-27835299-1');
</script>
<%
...
IF Not (tempFSO.FileExists(PathToTRA & "sql-debug-log.txt")) = true THEN
   Set logobject=tempFSO.CreateTextFile(PathToTRA & "sql-debug-dla-log.txt",true)
ELSE
   Set logobject=tempFSO.OpenTextFile(PathToTRA & "sql-debug-dla-log.txt",8,true)
END IF
