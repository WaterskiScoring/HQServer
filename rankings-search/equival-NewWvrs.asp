...
IF Request("Equival") <> "" THEN
	...
	IF trim(Request("skiyear")) <> "" THEN
		...
		ELSEIF trim(request("skiyear")) <= 16 THEN
...
sSQL = sSQL & " Coalesce(TS.[Region Code], '6') as Region,"
...
sSQL = sSQL & " CWS.WaiverStatusID"

sSQL = sSQL & " FROM Waterski.dbo.tblPeople PT, Waterski.dbo.[Membership History] MH,"

sSQL = sSQL & " (Select [Person ID] as PersonID, cast(substring(Max(convert(char(8),MH.EffectiveTo,112)"
sSQL = sSQL & "+right(convert(char(8),10000000+MH.MembershipHistoryID),7)),9,7) as integer) as MemHistID"
sSQL = sSQL & " From Waterski.dbo.[Membership History] MH"
sSQL = sSQL & " Join Waterski.dbo.[Current Waiver Status by Membership History ID] CWS"
sSQL = sSQL & " on CWS.MembershipHistoryID = MH.MembershipHistoryID"
sSQL = sSQL & " group by [Person ID]) MHL,"

sSQL = sSQL & " Waterski.dbo.[Current Waiver Status by Membership History ID] CWS,"

sSQL = sSQL & " Waterski.dbo.tblMembershipTypeCodes MT, Waterski.dbo.tblDivisionCodes D1,"
sSQL = sSQL & " Waterski.dbo.tblDivisionCodes D2, Waterski.dbo.tblPeopleAddresses PA"
sSQL = sSQL & " LEFT JOIN Waterski.dbo.tblStates TS ON PA.State = TS.[State Code]"

sSQL = sSQL & " WHERE PA.[Person ID] = PT.[Person ID] AND PA.[Primary] = 1"

sSQL = sSQL & " AND PT.[Person ID] = MH.[Person ID]"
sSQL = sSQL & " AND MH.[Person ID] = MHL.PersonID"
sSQL = sSQL & " AND MH.MembershipHistoryID = MHL.MemHistID"
sSQL = sSQL & " AND CWS.MembershipHistoryID = MHL.MemHistID"
