...
VBCrLf = Chr(13) & Chr(10): StartTime = Time(): DupMemList = "": ReCalc12 = "N"
...
EMailToWho = "<AWSATechDude@comcast.net>; <mark@productdesign-biz.com>; <shardee@usawaterski.org>"
EmailErrors = 0
...
DO UNTIL HQrs.eof
	...
	sSQL = sSQL & " Where PersonID = '" & HQrs("NewMemID") & "'" 
	rs.open sSQL, sConnectionToTRATable, 3, 3
	rs.MoveFirst
	j = rs("Kount")
	rs.close
...
WriteLog(date() &"  "& time() &"  Membership Was-to-Is Update Completed Successfully.")


' ************** Bottom of Member Extract / Update / Was/Is Conditional Section ***************
...
' But only for "Ranking Divisions", where Left(RS.Div,1) in ('B','G','I','J','M','W','O','C')
' Hence omits Novice -- But International Divisions are translated to Elite and/or Age divisions.

sSQL = sSQL & " SELECT RS.MemberID, RS.Event, CASE when RS.Div = 'IM' then 'OM' when RS.Div = 'IW' then 'OW'"
sSQL = sSQL & " when RS.Div = 'JB' then 'B2' when RS.Div = 'JG' then 'G2'"
sSQL = sSQL & " when RS.Div = 'IB' then 'B3' when RS.Div = 'IG' then 'G3'"
sSQL = sSQL & " when RS.Div = 'S1' then 'M3' when RS.Div = 'S2' then 'M4'"
sSQL = sSQL & " when RS.Div = 'S3' then 'M5' when RS.Div = 'L1' then 'W3' when RS.Div = 'L2' then 'W4'"
sSQL = sSQL & " when RS.Div = 'L3' then 'W5' else RS.Div end as Div, '2' as EStatus FROM " & RawScoresTableName
...
sSQL = sSQL & " (MemberID, Team, TeamStat, TourID, Div, Event, Round, Class, PrioClass, Place, ScoreOrig, DivOrig, Score, Rating, FmtScore, OAScore, SkiYearID)"
...
sSQL = "INSERT INTO " & EquivScoresTableName
sSQL = sSQL & " (MemberID, Team, TeamStat, TourID, Div, Event, Round, Class, PrioClass,"
sSQL = sSQL & " Place, ScoreOrig, DivOrig, Score, Rating, FmtScore, SkiYearID, OAScore)"
sSQL = sSQL & " SELECT ES.MemberID, ES.Team, ES.TeamStat, ES.TourID, DE.Div, ES.Event, ES.Round, ES.Class,"
sSQL = sSQL & " ES.PrioClass, ES.Place, ES.ScoreOrig, ES.DivOrig, ES.Score, ES.Rating, ES.FmtScore, ES.SkiYearID,"
...
sSQL = sSQL & " Case when XS.Event in ('S','T') then XS.Score when DE.Ramp1 > DO.Ramp1"
sSQL = sSQL & " then Case when DE.Max_J1 > DO.Max_J1 then XS.Score + 12 + (8 * ((DE.Max_J1-DO.Max_J1)/3)) else XS.Score + 12 end"
sSQL = sSQL & " else Case when DE.Max_J1 > DO.Max_J1 then XS.Score + (8 * ((DE.Max_J1-DO.Max_J1)/3)) else XS.Score end end as Score"
sSQL = sSQL & " FROM (SELECT MemberID, TourID, Event, Round, Class, DivOrig, Place, SkiYearID, Team, PrioClass, TeamStat,"
sSQL = sSQL & " Case when Max(Div) in ('W1','W2','OW') then Max(Div) else Min(Div) end as DivBase, Min(Score) as Score,"
sSQL = sSQL & " Min(FmtScore) as FmtScore, Max(Rating) as Rating, Max(CASE When Div in ('M1','M2','OM') then 'EM'"
sSQL = sSQL & " When Div in ('W1','W2','OW') then 'EW' When Div in ('M3','M4','MM') then 'SM' end ) as DivElite FROM "
sSQL = sSQL & EquivScoresTableName & " Where Div in ('M1','M2','OM','W1','W2','OW','M3','M4','MM') and	SkiYearID = " & sProcessingYear
...
sSQL = sSQL & " and Left(OA.TourID,6) = Left(ST.TournAppID,6)"
...
DO UNTIL rs.eof
  ...
  sSQL = sSQL & " VALUES ('" & TempMemberID & "','" & TempTeam & "', 'O', '" & TempDiv & "', '" & RSco1
  sSQL = sSQL & "', '" & RExp1 & "With " & RPen1 & "', " & sProcessingYear & ")"
...
sSQL = sSQL & " Max(ES.SkiYearID) as SkiYearID, Max(ST.TName) as TName,"
sSQL = sSQL & " Max(ES.Team) as Team, Max(ES.TeamStat) as TeamStat"
sSQL = sSQL & " FROM " & EquivScoresTableName & " as ES, " & SanctionTableName & " as ST"
sSQL = sSQL & " WHERE ES.SkiYearID = " & sProcessingYear 
sSQL = sSQL & " and Left(ES.TourID,6) = Left(ST.TournAppID,6)"
...
DO UNTIL rs.eof
  ...
  sSQL = "INSERT INTO " & RankTableName & " (MemberID, Team, TeamStat, Event, Div, SC_1, RankScore, RnkScoBkup, AWSA_Rat, OpenRating, Reg_SKI, Regl_Plc, Natl_Plc, SkiYearID)"
  sSQL = sSQL & " VALUES ('" & TempMemberID & "', '" & TempTeam & "','" & TempTeamStat & "','" & TempEvent & "', '" & TempDiv & "', '" & RSco1 & "', '" & RSco1 & "', '"
...
' tables, whose Timestamps are over 12 hours old.  Then also Reset the Placement 
...
'	USA skiers with ranking scores that equal or exceed the (possibly adjusted) Level 9 
'	cutoff score for their applicable division and event.  Three steps here ...

'	First we do all events EXCEPT Overall, for all age divisions.

sSQL = "UPDATE RT SET AWSA_Rat = CT.Event + '9' FROM " & RankTableName & " as RT, "
sSQL = sSQL & CutOffTableName & " as CT, " & MemberTableName & " as MT" 
sSQL = sSQL & " WHERE RT.MemberID = MT.PersonIDWithCheckDigit"
sSQL = sSQL & " AND UPPER(MT.FederationCode) = 'USA'"
sSQL = sSQL & " and CT.Div = RT.Div and CT.Event = RT.Event"
sSQL = sSQL & " and CT.SkiYearID = RT.SkiYearID and RT.SkiYearID = " & sProcessingYear
sSQL = sSQL & " and RT.Event <> 'O' and CT.COA9 <= RT.RankScore"

' WriteDebugSQL(sSQL)

Con.Execute(sSQL)

'	Next we do Overall for those divisions that fall in the Elite Pools, against the
'	overall scores as calculated for that respective Elite Pool.

sSQL = "UPDATE RT SET AWSA_Rat = CT.Event + '9' FROM " & RankTableName & " as RT, "
sSQL = sSQL & CutOffTableName & " as CT, " & MemberTableName & " as MT" 
sSQL = sSQL & " WHERE RT.MemberID = MT.PersonIDWithCheckDigit"
sSQL = sSQL & " AND UPPER(MT.FederationCode) = 'USA' and CT.Event = RT.Event and CT.Div ="
sSQL = sSQL & " CASE When RT.Div in ('M1','M2','OM') then 'EM' When RT.Div in ('W1','W2','OW')"
sSQL = sSQL & " then 'EW' When RT.Div in ('M3','M4','MM') then 'SM' end"
sSQL = sSQL & " and RT.Div in ('M1','M2','OM','W1','W2','OW','M3','M4','MM')"
sSQL = sSQL & " and CT.SkiYearID = RT.SkiYearID and RT.SkiYearID = " & sProcessingYear
sSQL = sSQL & " and RT.Event = 'O' and CT.COA9 <= RT.RankScore"
...
'	Finally we do Overall for SELECTED age divisions, outside the Elite Pools.

'	*********** This needs to have some suitable adjustment factored in, for
'	*********** differences between Elite overall and Age Div overall calcs.

sSQL = "UPDATE RT SET AWSA_Rat = CT.Event + '9' FROM " & RankTableName & " as RT, "
sSQL = sSQL & CutOffTableName & " as CT, " & MemberTableName & " as MT" 
sSQL = sSQL & " WHERE RT.MemberID = MT.PersonIDWithCheckDigit"
sSQL = sSQL & " AND UPPER(MT.FederationCode) = 'USA' and CT.Div = RT.Div"
sSQL = sSQL & " and CT.Event = RT.Event and RT.Div in ('B3','G3','M5','W3','W4')"
sSQL = sSQL & " and CT.SkiYearID = RT.SkiYearID and RT.SkiYearID = " & sProcessingYear
sSQL = sSQL & " and RT.Event = 'O' and CT.COA9 <= RT.RankScore"
...
'	Qualified Through Date table, for those qualifying today as Open or Masters.
'	Done in two major steps, first one for Open, second for Masters.

'	First major step does Open Division for all age ranges.
