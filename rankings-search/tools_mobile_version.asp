...
<head>
<meta charset="utf-8">
<title>AWSA Mobile Rankings Page Live</title>
...
SELECT CASE OpenState
		...
		CASE "mainmenu"
				'response.write("<div style=color:red; background-color:white;>Line 97 - tools_mobile_ver</div>")
			' response.end
...
'   End If
		...
		IF Instr(User_Agent,"iphone")>0 OR Instr(User_Agent,"ipod")>0 OR Instr(User_Agent,"ipad")>0 THEN
				Session("OperatingSystem") = "iOS"
				Session("devicetype") = "mobile"
		END IF
		IF Instr(User_Agent,"android")>0 THEN
				Session("OperatingSystem") = "Android"
				Session("devicetype") = "mobile"
		END IF
		IF Instr(User_Agent,"windows")>0 THEN
				Session("OperatingSystem") = "Windows"
		END IF
   

   ' response.write("<div style=color:red>In tools is_mobile = "&is_mobile&"</div>")
