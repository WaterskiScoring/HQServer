...
sSkiYearSelected = Request("SkiYearIDSelected")
' response.write("<br></div><div style='color:white;'>sSkiYearSelected = "&sSkiYearSelected)

IF TRIM(sSkiYearSelected)="" THEN sSkiYearSelected=1
...
' response.write("<br></div><div style='color:white;'>sSQL = "&sSQL)
' response.end
...
DO WHILE NOT rs.eof
		...
		'IF ArrayIndex=0 THEN SlalomExpArray = "("&OverExp_S ELSE SlalomExpArray = SlalomExpArray & "," &OverExp_S
		'IF ArrayIndex=0 THEN TrickExpArray = "("&OverExp_T ELSE TrickExpArray = TrickExpArray & "," &OverExp_T
		'IF ArrayIndex=0 THEN JumpExpArray = "("&OverExp_J ELSE JumpExpArray = JumpExpArray & "," &OverExp_J					
		
		'IF ArrayIndex=0 THEN OverPtsBySArray = "("&OverPtsBy_S ELSE OverPtsBySArray = OverPtsBySArray & "," &OverPtsBy_S				

		' --- Adding leading zero corrects problem of splitting an array with only one value 7/2/2016 ---
...
response.write("</div><div style='color:red;'> JumpNOPSArray = "&JumpNOPSArray)
response.write("</div><div style='color:red;'> JumpExpArray = "&JumpExpArray)
	...
	<div style="width:96%; margin-top:10px; padding-left:10px; text-align:left; border:0px solid red;">		
		...
		<div style="width:96%; margin:0px 0px 0px 0px; padding:0px 0px 0px 0px; border:0px solid white; text-align:left;">
			...
			<span class="span25" style="text-align:right;">
				<input type="number" id="RawScore_S" name="RawScore_S" value="<%=RawScore_S%>" style="font-size:12pt; width:50px; text-align:right;">
			</span>
		...
		<div style="width:96%; margin:15px 0px 0px 0px; padding:0px 0px 0px 0px; border:0px solid white; text-align:left;">
			...
			<span class="span25" style="text-align:right;">
				<input type="number" id="RawScore_T" name="RawScore_T" value="<%=RawScore_T%>" style="font-size:12pt; width:50px; text-align:right;">
			</span>
			<span class="span20" style="text-align:right;">
		...
		<div style="width:96%;  margin:15px 0px 0px 0px; padding:0px 0px 0px 0px; border:0px solid white; text-align:left;">
			...
			<span class="span25" style="text-align:right;">
				<input type="number" id="RawScore_J" name="RawScore_J" value="<%=RawScore_J%>" style="font-size:12pt; width:50px; text-align:right;">
			</span>
			<span class="span20" style="text-align:right;">
