...
EMailToWho = "<AWSATechDude@comcast.net>; <mark@productdesign-biz.com>; <mhanson@usawaterski.org>"
EmailErrors = 0
...
sSQL = sSQL & " when RS.Div = 'IB' then 'B3' when RS.Div = 'IG' then 'G3' when RS.Div = 'JB' then 'B2'"
sSQL = sSQL & " when RS.Div = 'JG' then 'G2' when RS.Div = 'S1' then 'M3' when RS.Div = 'S2' then 'M4'"
...
sSQL = sSQL & " + ES.PrioClass) as MaxScore, Max(ES.DivOrig) as DivOrig,"
sSQL = sSQL & " Max(ES.Place) as Place, Max(ES.Rating) as Rating,"
...
DO UNTIL rs.eof
  ...
  DO WHILE TempMemberID = rs("MemberID") AND TempDiv = rs("Div") AND TempEvent = rs("Event")
     ...
     IF TempEvent = "S" THEN FmtSco = FormatNumber(TempScore,2): ELSE FmtSco = FormatNumber(TempScore,0)

     IF left(TempDiv,1) = "C" THEN
     	
        ' NCWSA Logic -- nScoR counts # scores Recorded, nScoC counts # Scores Used.
        ' Include second score, unless avg drops below 75% of the best score alone.

        IF nScoR < 2 THEN     
           nScoR = nScoR + 1
           IF nScoR = 1 OR (RSco1+TempScore)/2 > .75*RSco1 THEN
              nScoC = nScoC + 1
  ...
  sSQL = "INSERT INTO " & RankTableName & " (MemberID, Team, Event, Div, SC_1, RankScore, RnkScoBkup, AWSA_Rat, OpenRating, Reg_SKI, Regl_Plc, Natl_Plc, SkiYearID)"
  sSQL = sSQL & " VALUES ('" & TempMemberID & "', '" & TempTeam & "','" & TempEvent & "', '" & TempDiv & "', '" & RSco1 & "', '" & RSco1 & "', '"
...
' within each division and event.
...
sSQL = sSQL & " Order by Div, Event, Team, RankScore"
