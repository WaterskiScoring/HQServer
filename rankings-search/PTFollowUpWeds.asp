...
Dim objRS, objMail, NumTours, NumEmails, sSQL
...
Dim eMailTo, eMailCC, eMailBody, strMissing, SeedRep, SeedFName

NumTours = 0: NumEmails = 0


%><p>Begin Post Tournament Followup processing for <%=FormatDateTime(date(),1)%>.</p><%

Set objRS = Server.CreateObject("ADODB.recordset")
Set objMail = CreateObject("CDO.Message")
objMail.From = """USA Water Ski Competition"" <tbaylon@usawaterski.org>"
...
sSQL = sSQL & " WHERE ST.TStatus in (2,4)"
sSQL = sSQL & " AND substring(ST.TournAppID,3,1) in ('C','E','M','S','W','U')"
sSQL = sSQL & " AND substring(ST.TSanction,7,1) in ('C','E','L','R','A','B')"
...
IF NOT objRS.eof THEN
	...
	DO Until objRS.eof
		...
		%><p><%=TSanction%>&nbsp;&nbsp; <%=TName%>&nbsp;&nbsp; (<%=TDateE%>, <%=WksOld%> wks old)<br><%
		...
		IF len(eMailTo) > 0 THEN
			...
			'	Then if we actually have primary address(es), 
			'	next we establish secondary addressing based on jurisdiction code

			objMail.To = eMailTo

			IF mid(TSanction,3,1) = "M" or mid(TSanction,3,1) = "U" THEN 
				objMail.BCC = ""
			ELSE
				objMail.BCC = """Dave Clark"" <awsatechdude@comcast.net>"
			END IF
			
			IF mid(TSanction,3,1) = "C" THEN
				eMailCC = """Bob Mayhew"" <skident@gmail.com>; ""Melissa Huitt"" <melissaskier@gmail.com>"
				SeedRep = "Melissa Huitt": SeedFname = "Melissa"
			ELSEIF mid(TSanction,3,1) = "M" or mid(TSanction,3,1) = "U" THEN
		   	eMailCC = """Jeff Surdej"" <j_surdej@yahoo.com>; ""Dave Clark"" <awsatechdude@comcast.net>"
				SeedRep = "Dave Clark": SeedFname = "Dave"
			ELSEIF mid(TSanction,3,1) = "E" THEN
		   	eMailCC = """Pat Byrne"" <pjbyrn@aol.com>; ""Jennifer Frederick-Kelly"" <fredmach@choiceonemail.com>"
				SeedRep = "Jennifer Frederick-Kelly": SeedFname = "Jennifer"
			ELSEIF mid(TSanction,3,1) = "S" THEN
		   	eMailCC = """Gene Davis"" <skijump@att.net>; ""Gary Pressley"" <garypressley@charter.net>"
				SeedRep = "Gary Pressley": SeedFname = "Gary"
			ELSEIF mid(TSanction,3,1) = "W" THEN
		   	eMailCC = """Barry Young"" <malski64@hotmail.com>; ""Laura Johnson"" <seeding@awsawest.org>"
				SeedRep = "Laura Johnson": SeedFname = "Laura"
			ELSE
			...
			%>Email to:&nbsp;&nbsp; <%=eMailTo%><br>CC To:&nbsp;&nbsp; <%=eMailCC%>&nbsp;&nbsp;
			...
			IF WksOld = 1 AND TStatus = 2 THEN
				...
				eMailBody = eMailBody & "The Zip file should be emailed to " & SeedRep & ", your Regional" & vbCRLF 
				eMailBody = eMailBody & "seeding committee representative, who is copied herein.  If you" & vbCRLF
				eMailBody = eMailBody & "are having difficulty producing any of those reports, or the Zip" & vbCRLF
				eMailBody = eMailBody & "file itself, please contact " & SeedFName & " for assistance." & vbCRLF & vbCRLF
			...
			ELSEIF TStatus = 2 THEN
				...
				eMailBody = eMailBody & "should be emailed to " & SeedRep & ", your Regional seeding" & vbCRLF
				eMailBody = eMailBody & "committee representative, who is copied herein.  If you are" & vbCRLF
				eMailBody = eMailBody & "having difficulty producing any of those reports, or the Zip" & vbCRLF
				eMailBody = eMailBody & "file itself, please contact " & SeedFName & " for assistance." & vbCRLF & vbCRLF
			...
			ELSE
				...
				eMailBody = eMailBody & "Electronic submission of these items to your Regional seeding" & vbCRLF 
				eMailBody = eMailBody & "representative, " & SeedRep & ", is preferred -- although you" & vbCRLF 
				eMailBody = eMailBody & "may submit direct to USA Waterski HQ by postal mail instead.  If" & vbCRLF 
				eMailBody = eMailBody & "you are having difficulty producing any of those reports, or the" & vbCRLF 
				eMailBody = eMailBody & "WSTIMS Zip file itself, please contact " & SeedFName & " for assistance." & vbCRLF & vbCRLF
	
			END IF

			eMailBody = eMailBody & "Thank you for your hard work and continued support," & vbCRLF & vbCRLF

			eMailBody = eMailBody & "Traci Baylon" & vbCRLF & "Competition Services Coordinator" & vbCRLF
			eMailBody = eMailBody & "tbaylon@usawaterski.org" & vbCRLF & "1-863-324-4341 ext 121"
		...
		ELSE
			%>No eMail addresses in Sanction Tables for this tournament.</p><%
		END IF
...
%>
		...
		<p>E-Mail Followups sent for <%=NumEmails%> Tournaments.</p>
