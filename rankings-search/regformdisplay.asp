...
Dim sTEntryFee1DisplayR, sTEntryFee2DisplayR, sTEntryFee3DisplayR
Dim sTEntryFee1DisplayS, sTEntryFee2DisplayS, sTEntryFee3DisplayS
Dim sGREntryFee1Display, sGREntryFee2Display, sGREntryFee3Display

'response.write("<br>Line 224 - rdf sTourID = "&sTourID)
IF sTourID="16C017" OR LEFT(sTourID,6)="16M014" THEN
		sTEntryFee1DisplayR = sTEntryFee1 + sRSurchg
		sTEntryFee2DisplayR = sTEntryFee2 + sRSurchg
		sTEntryFee3DisplayR = sTEntryFee3 + sRSurchg
ELSE
		sTEntryFee1DisplayR = sTEntryFee1 + sRSurchg
		sTEntryFee2DisplayR = sTEntryFee2
		sTEntryFee3DisplayR = sTEntryFee3
END IF	

sGREntryFee1Display = sGREntryFee1
sGREntryFee2Display = sGREntryFee2-sGREntryFee1
sGREntryFee3Display = sGREntryFee3-sGREntryFee1

sTEntryFee1DisplayS = sTEntryFee1
sTEntryFee2DisplayS = sTEntryFee2
sTEntryFee3DisplayS = sTEntryFee3
	...
	<tr><%
		...
		IF Cdbl(sGREntryFee1)>cdbl(0.00) THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sGREntryFee1Display,2) %></FONT></TD><%
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF

		IF cdbl(sTEntryFee1)>cdbl(0.00) THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sTEntryFee1DisplayS,2) %></FONT></TD><%
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF

		IF sRSurchg>0 THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sTEntryFee1DisplayR,2) %></FONT></TD><%		
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF %>
...
'IF MarkTester THEN response.write("<br>sTEntryFee2="& sTEntryFee2)
		...
		IF Cdbl(sGREntryFee2)>cdbl(0.00) THEN %>		
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sGREntryFee2Display,2) %></FONT></TD><%
		...
		IF Cdbl(sTEntryFee2)>0.00 THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sTEntryFee2DisplayS,2) %></FONT></TD><%		
		ELSE %>
			<TD>&nbsp;</td><%				
		END IF
		...
		IF sRSurchg>0 AND Cdbl(sTEntryFee2)>cdbl(0.00) THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sTEntryFee2DisplayR,2) %></FONT></TD><%		
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF %>
	...
	<tr><%
		...
		IF Cdbl(sGREntryFee3)>0.00 THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sGREntryFee3Display,2) %></FONT></TD><%		
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF

		IF Cdbl(sTEntryFee3)>cdbl(0.00) THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sTEntryFee3DisplayS,2) %></FONT></TD><%		
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF
		' --- Was sTEntryFee3+sRSurchg until 7/16/2014 
		IF sRSurchg>0 AND Cdbl(sTEntryFee3)>cdbl(0.00) THEN %>
				<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sTEntryFee3DisplayR,2) %></FONT></TD><%		
		ELSE %>
				<TD ALIGN="center">&nbsp;</td><%
		END IF %>
	      ...
	      <tr>
		    	<td colspan="4" style="height:2em;">
		    		<input type=checkbox name="fAWSEFCheck" <% IF sAWSEFCheck = "on" THEN Response.Write("Checked "&AllObjectStatus) ELSE Response.write(AllObjectStatus) %>>
		   			<font size=<% =fontsize2 %> face=<% =font1 %>>&nbsp;&nbsp;Check to Donate $10.00 to USA Water Ski Foundation</font>
		    	</td>
		    	<%
