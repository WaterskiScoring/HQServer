	...
	IF NOT objRS.eof THEN
		...
		IF len(eMailTo) > 0 THEN
			...
			ELSEIF mid(TSanction,3,1) = "E" THEN
		   	eMailCC = """Tonya Maurer"" <multitasking@comcast.net>"
			...
			ELSEIF mid(TSanction,3,1) = "S" THEN
		   	eMailCC = """Bob Schouten"" <evp@awsasouth.org>"
			...
			ELSE
		   	...
		   	eMailFrom = """USA Water Ski Competition"" <mstone@usawaterski.org>"
				SeedRep = "Matthew Stone" & vbCRLF & "Competition & Events Coordinator" & vbCRLF & "mstone@usawaterski.org" & vbCRLF & "1-863-324-4341 ext 121"
			END IF
...
Dim TotOASc, ROAS1, ROAS2, ROAS3, FmtOASc
...
sSQL = sSQL & " Case when Max(Div) in ('W1','W2','W3','W4','W5','OW') then Min(Div) else Max(Div) end as DivBase, Max(Score) as Score,"
sSQL = sSQL & " Max(FmtScore) as FmtScore, Max(Rating) as Rating, Max(CASE When Div in ('B2','B3','M1','M2','M3','M4','M5','OM','MM')"
sSQL = sSQL & " then 'EM' else 'EW' end) as DivElite FROM " & EquivScoresTableName & " Where Div in"
sSQL = sSQL & " ('B2','B3','M1','M2','M3','M4','M5','OM','MM','G2','G3','W1','W2','W3','W4','W5','OW','MW') and SkiYearID = " & sProcessingYear
...
'	Second step here inserts SM/SW from possible candidates, Coding as DivType C or D, 
...
sSQL = sSQL & " else XS.Score + (12 * ((DE.Ramp1-DO.Ramp1)/.020)) + (8 * ((DE.Max_J1-DO.Max_J1)/3)) end as Score"
sSQL = sSQL & " FROM (SELECT MemberID, TourID, Event, Round, Class, DivOrig, Place, SkiYearID, Team, PrioClass, TeamStat,"
sSQL = sSQL & " Min(Div) as DivBase, Min(Score) as Score, Min(FmtScore) as FmtScore, Max(Rating) as Rating, Max(CASE When Div in"
sSQL = sSQL & " ('M3','M4','M5','M6','M7','MM') then 'SM' else 'SW' end) as DivElite FROM "
sSQL = sSQL & EquivScoresTableName & " Where Div in ('M3','M4','M5','M6','M7','MM','W3','W4','W5','W6','MW') and SkiYearID = " & sProcessingYear
sSQL = sSQL & " Group by 	MemberID, TourID, Event, Round, Class, DivOrig, Place, SkiYearID, Team, PrioClass, TeamStat) as XS, "
sSQL = sSQL & DivisionsTableName & " as DO, " & DivisionsTableName & " as DE Where DO.Div = XS.DivBase"
sSQL = sSQL & " and DO.SkiYearID = XS.SkiYearID and	DE.Div = XS.DivElite and DE.SkiYearID = XS.SkiYearID) as ES, "
sSQL = sSQL & DivisionsTableName & " as DE WHERE DE.Div = ES.Div and DE.SkiYearID = ES.SkiYearID"
...
DO UNTIL rs.eof
  ...
  nScoC = 0: nScoR = 0: RSco1 = 0: RSco2 = 0: RSco3 = 0: TotScore = 0
  ROAS1 = 0: ROAS2 = 0: ROAS3 = 0: TotOASc = 0
     ...
     ELSE
        ...
        IF nScoC+nScoR < 3 THEN     
           ...
           IF nScoC+nScoR = 1 THEN
              ...
              RExp1 = RExp1 & " from " & SQLClean(rs("TName")) & "&#13;"
              RSco1 = (1-(tPenalty(nScoC,nScoR)/100))*(TotScore)/(nScoC+nScoR)
              ROAS1 = (1-(tPenalty(nScoC,nScoR)/100))*(TotOASc)/(nScoC+nScoR)
              IF tPenalty(nScoC,nScoR) > 0 THEN RPen1 = tPenalty(nScoC,nScoR) & "% Penalty": ELSE RPen1 = "NO Penalty"
           ...
           ELSEIF nScoC+nScoR = 2 THEN
              ...
              RExp2 = RExp2 & " from " & SQLClean(rs("TName")) & "&#13;"
              RSco2 = (1-(tPenalty(nScoC,nScoR)/100))*(TotScore)/(nScoC+nScoR)
              ROAS2 = (1-(tPenalty(nScoC,nScoR)/100))*(TotOASc)/(nScoC+nScoR)
              IF tPenalty(nScoC,nScoR) > 0 THEN RPen2 = tPenalty(nScoC,nScoR) & "% Penalty": ELSE RPen2 = "NO Penalty"
           ...
           ELSE
              ...
              RExp3 = RExp3 & " from " & SQLClean(rs("TName")) & "&#13;"
              RSco3 = (1-(tPenalty(nScoC,nScoR)/100))*(TotScore)/(nScoC+nScoR)
              ROAS3 = (1-(tPenalty(nScoC,nScoR)/100))*(TotOASc)/(nScoC+nScoR)
              IF tPenalty(nScoC,nScoR) > 0 THEN RPen3 = tPenalty(nScoC,nScoR) & "% Penalty": ELSE RPen3 = "NO Penalty"
...
' WriteDebugSQL("Begin Ranking Finalization " & TempMemberID & " / " & TempDiv & " / " & TempEvent & " / " & ROAS1 & " / " & ROAS2 & " / " & ROAS3)
  ...
  ELSE
     ...
     IF RSco2 >= RSco1 THEN
        RSco1 = RSco2: ROAS1 = ROAS2: RExp1 = RExp2: RPen1 = RPen2
     ELSE
        IF nScoC+nScoR = 2 THEN RPen1 = RPen1 & "&#13;Rule 1.13 Applied; see FAQ/Tips"
     END IF

     IF RSco3 >= RSco1 THEN
        RSco1 = RSco3: ROAS1 = ROAS3: RExp1 = RExp3: RPen1 = RPen3
     ELSE
        IF nScoC+nScoR > 2 THEN RPen1 = RPen1 & "&#13;Rule 1.13 Applied; see FAQ/Tips"
     END IF
  ...
  FmtOASc = FormatNumber(ROAS1,1): RPen1 = RPen1 & "&#13;( NOPS = " & FmtOASc & " )"
...
'	First sub-step does OPEN for all divisions all ages, except NOT overriding EM/EW/SM/SW of course.
...
sSQL = sSQL & " and DA.Div not in ('EM','EW','SM','SW') and DE.SkiYearID = " & sProcessingYear
...
'	we do Overall ONLY for the Elite Pseudo-Divs EM/EW/SM/SW.
...
if sProcessingYear = 1 THEN
	...
	sSQL = sSQL & " and RT.Div in ('EM','EW','SM','SW') and EQD.DivElite = CASE"
	sSQL = sSQL & " when RT.Div = 'EM' then 'OM' when RT.Div = 'EW' then 'OW'"
	sSQL = sSQL & " when RT.Div = 'SM' then 'MM' when RT.Div = 'SW' then 'MW' end"
	sSQL = sSQL & " and EQD.Event = left(RT.Event,1) and right(RT.AWSA_Rat,1) = '9'"
	...
	sSQL = sSQL & " and EQD.DivElite = CASE when RT.Div = 'EM' then 'OM' when RT.Div = 'EW' then 'OW'"
	sSQL = sSQL & " when RT.Div = 'SM' then 'MM' when RT.Div = 'SW' then 'MW' end"
	...
	sSQL = sSQL & " in ('EM','EW','SM','SW') and RT.AWSA_Rat = left(RT.Event,1) + '9'"
...
'	Next we set up to produce the emails.
	...
	objMessage.From = """AWSA President"" <j_surdej@yahoo.com>"
'	objMessage.From = """AWSA President"" <SkiDent@gmail.com>"

'	objMessage.BCC = """Dave Clark"" <awsatechdude@comcast.net>; ""Bob Mayhew"" <SkiDent@gmail.com>; ""Keith Lindemulder"" <keith.lindemulder@nucor.com>"
	objMessage.BCC = """Jeff Surdej"" <j_surdej@yahoo.com>; ""Keith Lindemulder"" <keith.lindemulder@nucor.com>; ""Dave Clark"" <awsatechdude@comcast.net>"
...
'			objMessage.To = """Dave Clark"" <awsatechdude@comcast.net>; ""Jeff Surdej"" <j_surdej@yahoo.com>; ""Keith Lindemulder"" <keith.lindemulder@nucor.com>"
