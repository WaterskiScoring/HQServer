	...
	IF NOT objRS.eof THEN
		...
		IF len(eMailTo) > 0 THEN
			...
			ELSEIF mid(TSanction,3,1) = "E" THEN
		   	...
		   	eMailFrom = """Jennifer Frederick-Kelly"" <fredmach@choiceonemail.com>": eMailBCC = eMailFrom
				SeedRep = "Jennifer Frederick-Kelly" & vbCrLf & "AWSA East Seeding" & vbCrLf & "fredmach@choiceonemail.com" & vbCrLf & "(716) 892-1425"
			ELSEIF mid(TSanction,3,1) = "S" THEN
		   	eMailCC = """Bob Archambeau"" <evp@awsasouth.org>"
		   	eMailFrom = """Gary Pressley"" <garypressley@charter.net>": eMailBCC = eMailFrom
			...
			ELSEIF mid(TSanction,3,1) = "W" THEN
		   	eMailCC = """Barry Young"" <malski64@hotmail.com>"
		   	eMailFrom = """Laura Johnson"" <seeding@awsawest.org>": eMailBCC = eMailFrom
				SeedRep = "Laura Johnson" & vbCrLf & "AWSA West Seeding" & vbCrLf & "seeding@awsawest.org" & vbCrLf & "(951) 522-3080"
			...
			ELSE
		   	...
		   	eMailFrom = """USA Water Ski Competition"" <tbaylon@usawaterski.org>"
				SeedRep = "Traci Baylon" & vbCRLF & "Competition Services Coordinator" & vbCRLF & "tbaylon@usawaterski.org" & vbCRLF & "1-863-324-4341 ext 121"
			END IF
			...
			ELSE
				...
				eMailBody = eMailBody & "Traci Baylon at USA Waterski HQ, by postal mail.  If you are having" & vbCrLf 
...
Dim nConsMems, nConsHits, nScoUpdts, nOvrUpdts, nRnkUpdts, nOffUpdts, nNewElites
...
nScoUpdts = 0: nOvrUpdts = 0: nRnkUpdts = 0: nOffUpdts = 0: nNewElites = 0
...
' ********* First step is to empty local ConsolidatedMembers table, then
...
DO UNTIL HQrs.eof
	...
	' New Logic Added 2010-11-23 -- add this entry to the new ConsolidatedMembers table

	sSQL = "INSERT INTO " & ConsMemTableName & " VALUES ("
	sSQL = sSQL & HQrs("OldMemID") & "," & HQrs("NewMemID") & ",'" 
	sSQL = sSQL & PersonIDwChkDgt(HQrs("OldMemID")) & "','"
	sSQL = sSQL & PersonIDwChkDgt(HQrs("NewMemID")) & "'," & j & ")"
	Con.Execute(sSQL)
	...
	IF j > 0 THEN
		...
		' "NewMemID" record found -- so now begin updating data tables
		
		' First step is to update any score table records that match "OldMemID"
...
IF Request("Equival") <> "" THEN 
	...
	Response.write(Formatnumber(nConsHits,0) & " ConsMems Rows hit to Updated Member Table<br>")
