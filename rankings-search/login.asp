...
' ---------------------------- LOGIN VERIFICATION --------------------------------
  ...
  if session("reallogin") = "Valid" then
      ...
      End If
'      If trim(request("username")) = "SEEDING" and trim(request("password")) = "GNIDEES" Then
'        TRALogin = 1
'        session("UserName") = "Seeding"
'        session("FirstName") = "Seeding"
'        session("LastName") = "User"
'        session("UserLevel") = 49
'        Session("adminmenulevel") = 40
'      End If
'      If trim(request("username")) = "SCORER" and trim(request("password")) = "REROCS" Then
'        TRALogin = 1
'        session("UserName") = "Scorer"
'        session("FirstName") = "Scoring"
'        session("LastName") = "User"
...
'      End If
      ...
      Else
        ...
        If rs.EOF Then
          ...
          %>
           ...
           Please Try Again.
           <br><br>
           <form action="/rankings/login.asp" method="post">
      ...
      ' 
      ' The USERLEVEL and the ADMINMENULEVEL
      ' The Userlevel is our protection to make sure people arent
      ' trying to make up random menu values
      '
      ' The AdminMenuLevel is what tells the system
      ' which menu to display.
      '
      ' If you try to display a menu that is beyond your 
      ' hidden userlevel value, then it will kick you out.
      '
      ' Most of the individual procedures also check your
      ' hidden userlevel just to make sure you didn't try
      ' to call them directly without using a menu.
