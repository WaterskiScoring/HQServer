...
Dim tTeam, tTeamStat, tNCWRegn, tNCWConf, LastRegion
...
ELSE   
   ...
   ELSEIF DivSelected = "OM" or DivSelected = "OW" or DivSelected = "MM" THEN
      tEliteStat = "None"
...
SET rs=Server.CreateObject("ADODB.recordset")
    ...
    sSQL = sSQL & " RT.*, TT.NCWRegion, TT.NCWConf,"
...
<tr>
  ...
  <td colspan=2>	
	...
	' --- AWSA does not display Current Ski Year Rankings
	IF sRunByWhat <> "NSL" AND sRunByWhat <> "NCWSA" THEN
...
'	response.write("<br>DN="&sDefaultNationals&"<br>")
'END IF
...
'response.write("<br>2")
 	ELSEIF TRIM(rs("TourStatus"))<>"X" AND LCASE(TRIM(sLeagueSelected))="none" AND LCASE(TRIM(sLeagueSelected))<>"" THEN %>
		<a href="/rankings/MemberQualifications.asp?sMemberID=<%=tMemberID%>&sTourID=<%=sDefaultNationals%>" title="Check details Qualifications for various Tournament/Leagues for this Member." target="_blank">
