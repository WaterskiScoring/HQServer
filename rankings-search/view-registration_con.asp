...
Dim ThisFileName
ThisFileName="view-registration_con.asp"
...
sSQL = sSQL + "	ON TGEN.TournAppID = LEFT(LT.QualifyTour,6)" 
	...
	' -- Modified 8-10-2017 at 7:55p ET - Per search recommendation --
	'rs.Close
'	Set rs = Nothing
'	Set rs=Server.CreateObject("ADODB.recordset")
' 	rs.open sSQL, sConnectionToTRATable, 3, 1
...
'rs.CursorType = AdoEnums.CursorType.STATIC  '3
'rs.LockType = AdoEnums.LockType.READONLY '1
'rs.Open comm
'CloseCon_special 

' -- Thomas note --
' -- In my first set of code, I had
' -- Set comm = Server.CreateObject("ADODB.recordset")
' -- which should read
' -- Set comm = Server.CreateObject("ADODB.Command")
...
SUB OpenCon_special
     ...
     Con.CommandTimeout = 60 'This is in seconds. 3000 seconds would be 50 minutes
END SUB
...
sSQL = sSQL + " ( SELECT Div FROM "&DivisionsTableName& " GROUP BY Div ) d"
' WHERE LOWER(LEFT(Div,1)) IN ('b','g','m','w','o','i') 

sSQL = sSQL + " LEFT JOIN" 
sSQL = sSQL + " (SELECT DIV, COUNT(MemberID) AS EVT1_CNT, SUM(FeeRounds) AS EVT1_SUM"
sSQL = sSQL + " FROM "&RegDetailTableName&" AS EVT" 
sSQL = sSQL + " WHERE Event='"&sTEvent(1)&"' AND LEFT(TourID,6)='"&LEFT(sTourID,6)&"'" 
sSQL = sSQL + " GROUP BY Div) AS EVT1"
sSQL = sSQL + " ON EVT1.Div=d.Div"

sSQL = sSQL + " LEFT JOIN "
sSQL = sSQL + " (SELECT div, COUNT(MemberID) AS EVT2_CNT, SUM(FeeRounds) AS EVT2_SUM"
sSQL = sSQL + " FROM "&RegDetailTableName&" AS EVT"
sSQL = sSQL + " WHERE Event='"&sTEvent(2)&"' AND LEFT(TourID,6)='"&LEFT(sTourID,6)&"'" 
sSQL = sSQL + " GROUP BY Div) AS EVT2"
sSQL = sSQL + " ON EVT2.DIV=d.DIV"

sSQL = sSQL + " LEFT JOIN "
sSQL = sSQL + " (SELECT div, COUNT(MemberID) AS EVT3_CNT, SUM(FeeRounds) AS EVT3_SUM"
sSQL = sSQL + " FROM "&RegDetailTableName&" AS EVT"
sSQL = sSQL + " WHERE Event='"&sTEvent(3)&"' AND LEFT(TourID,6)='"&LEFT(sTourID,6)&"'"
sSQL = sSQL + " GROUP BY Div) AS EVT3"
sSQL = sSQL + "  ON EVT3.DIV=d.DIV"
sSQL = sSQL + " WHERE COALESCE(EVT1.EVT1_CNT,0)>0 OR COALESCE(EVT2.EVT2_CNT,0)>0 OR COALESCE(EVT3.EVT3_CNT,0)>0"
...
IF NOT rsTot.eof THEN
	...
	<TABLE align="Center" class="innertable" width="<%=TourTableWidth%>px">
	  ...
	  DO WHILE Not rsTot.EOF 
		...
		IF ISNULL(ThisMetric_EVT1) THEN
				DivTimeEvt1 = fix(ThisMetric_EVT1 * EVT1_TIME/60)&":00"
		ELSE
				' DivTimeEvt1 = fix(ThisMetric_EVT1 * EVT1_TIME/60)&":"&formatNumber((((ThisMetric_EVT1 * EVT1_TIME/60) - fix(ThisMetric_EVT1 * EVT1_TIME/60))*60),00)
				DivTimeEvt1 = fix(ThisMetric_EVT1 * EVT1_TIME/60)	
		END IF



		IF ISNULL(ThisMetric_EVT2) THEN
		...
		ELSE
				...
				DivTimeEvt2 = fix(ThisMetric_EVT2 * EVT2_TIME/60)
		END IF

' response.write("<br>Sesssion "&Session("adminmenulevel"))

t=1
IF t=2 AND Session("adminmenulevel")>=50 THEN 
		response.write("<br>ThisMetric_EVT3 = "&ThisMetric_EVT1)
		response.write("<br>EVT3_TIME ="&EVT1_TIME)
		response.write("<br>First= "&(ThisMetric_EVT3 * EVT3_TIME/60))
		response.write("<br>Fix= "&fix(ThisMetric_EVT3 * EVT3_TIME/60))
	
		'response.write("<br>Next= "&((ThisMetric_EVT3 * EVT3_TIME/60) - fix(ThisMetric_EVT3 * EVT3_TIME/60)))
		'response.write("<br>Next2= "&(((ThisMetric_EVT3 * EVT3_TIME/60) - fix(ThisMetric_EVT3 * EVT3_TIME/60))*60))
	 	' response.end
	 ' --- PROBLEM STATEMENT BELOW ---
		'response.write("<br>Next3= "&formatNumber((((ThisMetric_EVT3 * EVT3_TIME/60) - fix(ThisMetric_EVT3 * EVT3_TIME/60))*60),0))

		
		'response.write("<br>")
		'response.write(NOT(LEFT(formatNumber((((ThisMetric_EVT3 * EVT3_TIME/60) - fix(ThisMetric_EVT3 * EVT3_TIME/60))*60),0),2) > cdbl(0)))
		' response.end
END IF
		 
		' IF NOT(LEFT(formatNumber((((ThisMetric_EVT3 * EVT3_TIME/60) - fix(ThisMetric_EVT3 * EVT3_TIME/60))*60),0),2) > cdbl(0)) THEN
		IF ISNULL(ThisMetric_EVT3) THEN 
		...
		ELSE
				...
				DivTimeEvt3 = fix(ThisMetric_EVT3 * EVT3_TIME/60)
		END IF
