...
Dim objFSO, objZip, objRS, objMail, eMailFrom, eMailTo, eMailCC, SeedRep, Owner
...
' Note -- we have WSPARM data only if we're handling a ZIP file.
...
strFileName = strTourID & "CJ.PDF"
strDescription = "Chief Judges Tournament Report"
ExtractFile strFileName, strTourFldr, 155000
IF objFSO.FileExists (strTourFldr & "\" & strFileName) THEN
	IF Instr(strAction, "Stored") > 0 THEN
		tmpFiles = objZip.ZipFilesTo(strTourFldr&"\"&strFileName, strTourZip)
		PTF_CJ = 1
...
strFileName = strTourID & "SD.PDF"
strDescription = "Safety Directors Report"
ExtractFile strFileName, strTourFldr, 280000
'	ExtractFile strFileName, strTourFldr, 140000
IF objFSO.FileExists (strTourFldr & "\" & strFileName) THEN
	IF Instr(strAction, "Stored") > 0 THEN
		tmpFiles = objZip.ZipFilesTo(strTourFldr&"\"&strFileName, strTourZip)
		PTF_SD = 1
...
strFileName = strTourID & "TU.PDF"
strDescription = "Towboat Utilization Report"
IF Right(Session("strTourDate"),4) < 2010 THEN
	strTUMin = 120000
ELSE
	strTUMin = 166000
END IF
ExtractFile strFileName, strTourFldr, strTUMin
IF instr(strAction,"Stored") > 0 THEN 
	objFSO.CopyFile strTourFldr & "\" & strFileName, PathtoHQInBox & "\", TRUE
END IF
IF objFSO.FileExists (strTourFldr & "\" & strFileName) THEN
...
IF instr("ELRPAB",mid(Session("strTourID"),7,1)) > 0 THEN
	...
	ExtractFile strFileName, strTourFldr, 6500
	IF instr(strAction,"Stored") > 0 THEN 
		objFSO.CopyFile strTourFldr & "\" & strFileName, PathtoTiming & "\", TRUE
	END IF
	IF objFSO.FileExists (strTourFldr & "\" & strFileName) THEN
...
strFileName = "WSPARM.TNY"
strDescription = "Tournament Settings"
ExtractFile strFileName, strTourFldr, 120
IF objFSO.FileExists (strTourFldr & "\" & strFileName) THEN
	IF Instr(strAction, "Stored") > 0 THEN
		tmpFiles = objZip.ZipFilesTo(strTourFldr&"\"&strFileName, strTourZip)
		PTF_TNY = 1
...
IF len(eMailTo) > 0 THEN
	...
	'objMail.Subject = "Post-Tournament Reports from " & Session("strTourID") & " " & Session("strTourName") & " (" & Session("strTourDate") & ")"

	IF mid(strTourID,3,1) = "C" THEN
		Owner = """Melissa Huitt"" <melissaskier@gmail.com>"
	ELSEIF mid(strTourID,3,1) = "M" or mid(TSanction,3,1) = "U" THEN
	...
	IF Session("Firstname") & Session("LastName") = "MelissaHuitt" THEN
		eMailCC = """Dave Clark"" <awsatechdude@comcast.net>"
		IF instr("CMU",mid(strTourID,3,1)) = 0 and len(Owner) > 0 THEN eMailCC = eMailCC & "; " & Owner
		eMailFrom = """Melissa Huitt"" <melissaskier@gmail.com>"
	...
	ELSEIF Session("Firstname") & Session("LastName") = "DaveClark" THEN
		eMailCC = """Dave Clark"" <awsatechdude@comcast.net>"
		IF instr("MU",mid(strTourID,3,1)) = 0 and len(Owner) > 0 THEN eMailCC = eMailCC & "; " & Owner
	...
	ELSEIF Session("Firstname") & Session("LastName") = "JenniferFrederick-Kelley" THEN
		eMailCC = """Dave Clark"" <awsatechdude@comcast.net>"
		IF instr("EMU",mid(strTourID,3,1)) = 0 and len(Owner) > 0 THEN eMailCC = eMailCC & "; " & Owner
	...
	ELSEIF Session("Firstname") & Session("LastName") = "GaryPressley" THEN
   	...
   	eMailFrom = """Gary Pressley"" <garypressley@charter.net>"
		SeedRep = "Gary Pressley" & vbCrLf & "AWSA South Seeding" & vbCrLf & "garypressley@charter.net" & vbCrLf & "(828) 301-1863"
	...
	ELSEIF Session("Firstname") & Session("LastName") = "JudyStanford" THEN
		eMailCC = """Dave Clark"" <awsatechdude@comcast.net>"
		IF instr("WMU",mid(strTourID,3,1)) = 0 and len(Owner) > 0 THEN eMailCC = eMailCC & "; " & Owner
	...
	ELSE
   	...
   	eMailFrom = """USA Water Ski Competition"" <bwolf@usawaterski.org>"
		SeedRep = Session("Firstname") & " " & Session("LastName") & " on behalf of Brandon Wolf" & vbCRLF & "Director of Competition" & vbCRLF & "bwolf@usawaterski.org" & vbCRLF & "1-863-324-4341 ext 120"
	END IF

	IF mid(strTourID,3,1) = "U" THEN
   	eMailCC = eMailCC & "; ""Jeff Surdej"" <j_surdej@yahoo.com>; ""Mike Mayleben"" <mike.mayleben@joomgo.com>; ""Cris Kodiak"" <cris.kodiak@gmail.com>"
	...
	ELSE
		...
		IF Session("UploadMode") = "Zip" THEN
			...
			eMailBody = eMailBody & "soon.  Submission of these to me as Electronic files via eMail is" & vbCRLF 
	...
	IF Instr(strMissing, "CS.HTM") = 0 THEN
		eMailBody = eMailBody & vbCRLF & vbCRLF & "WSTIMS (DOS) Scorebook can be viewed at: " & vbCRLF
	...
	'added my MOK for server move to IIS7 4-14-2013
	Set objMessage = CreateObject("CDO.Message")
	objMessage.Subject = "USA Waterski Error Message V1.2"
	objMessage.From = eMailFrom
	objMessage.To = eMailTo
	objMessage.cc = eMailCC
	objMessage.bcc = """Dave Allen"" <allendbj@comcast.net>"
	...
	' Now append this message to the eMails.txt file, creating if not already present.
