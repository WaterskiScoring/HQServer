...
Dim ThisFileName, StandingsFileName
ThisFileName="Search-MemberHQ.asp"
StandingsFileName = "view-standingsHQ.asp"



Dim currentPage, sMemberID, sLastName, sFirstName, RunByWhat, sSendingPage, pVar
Dim adminmenulevel, sUserAdminPW

Dim Ebody, sIncludeClubs
Dim DialogHeader

Dim FormStatus
Dim sTname, sAddressList, sEmailText, sTempPW, sPassword, sTourAdminPW, NowDate, sTableStatus, sTableEmail, sTableAC
Dim DisplayHeadFoot
...
'IF TRIM(sTourAdminPW)="" THEN sTourAdminPW="ZYXW2"
...
IF TRIM(adminmenulevel)="" THEN 
		adminmenulevel=0
'ELSE
'		adminmenulevel=1
END IF	

ErrorSubject="INFO - Line 84 - Search-MemberHQ - Session(adminmenulevel)="&Session("adminmenulevel")
ErrorSQL = "Session(sMemberID) = "&Session("sMemberID")
...
ELSEIF LCASE(LEFT(sSendingPage,22)) = "/rankings/registration" THEN
		PWreq = "yes"
ELSEIF LCASE(LEFT(sSendingPage,27)) = "/rankings/bio-form_launcher" THEN
		PWreq = "yes"		
END IF

'IF sMemberID="000001151" THEN 
'		response.write("<br>PWreq = "&PWreq)
'		response.write("<br>LCASE(LEFT(sSendingPage,22)) = "&LCASE(LEFT(sSendingPage,22)))	
		' response.end
'END IF
...
ELSEIF FormStatus = "confirmed" THEN
		...
		ErrorSubject="INFO - Line 160 - Search-MemberHQ - TRIM(sMemberID)="&TRIM(sMemberID)
		ErrorSQL = "sTourID = "&sTourID& " - sTourAdminPW = "&sTourAdminPW&" - sUserAdminPW = "&sUserAdminPW&" - adminmenulevel = "&adminmenulevel
		'SendErrorEmailToMark ErrorSubject, ErrorSQL
		...
		ErrorSubject="INFO - Line 175 - Search-MemberHQ - Session(adminmenulevel)="&Session("adminmenulevel")
		ErrorSQL = "Session(sMemberID) = "&Session("sMemberID")
		'SendErrorEmailToMark ErrorSubject, ErrorSQL
		...
		' IF sMemberID="000001151" THEN 
		' 		response.write("<br><br>Line 185 - PWreq = "&PWreq)
		' 		response.write("<br>LCASE(LEFT(sSendingPage,22)) = "&LCASE(LEFT(sSendingPage,22)))
				'response.write("<br>LCASE(sMemberPW) = "&LCASE(sMemberPW))
		...
		IF PWreq="yes" THEN
				...
				' IF sMemberID="000001151" THEN 
							' response.write("<br><br>Line 197")
							' response.write("<br>LCASE(LEFT(sSendingPage,22)) = "&LCASE(LEFT(sSendingPage,22)))
							' response.write("<br>sSendingPage = "&sSendingPage)
							' response.write("<br>adminmenulevel = "&adminmenulevel)
		...
		ELSEIF LCASE(LEFT(sSendingPage,27)) = "/rankings/bio-form_launcher" THEN
				sSendingPage = sSendingPage&"?sMemberID="&sMemberID
				Session("sMemberID") = sMemberID
				' IF sMemberID="000001151" THEN response.end
				
				response.redirect(sSendingPage)

		ELSEIF LCASE(LEFT(sSendingPage,27)) = "/rankings/register_emailmes" THEN
				sSendingPage = sSendingPage&"?sMemberID="&sMemberID
				Session("sMemberID") = sMemberID
				response.redirect(sSendingPage)
...
IF TRIM(sMemberID) <> "" THEN
		...
		END IF	
ELSE
		ErrorSubject="ERROR - Single Ranking Query Line 341 - Search-MemberHQ - TRIM(sMemberID) IS NULL"
		ErrorSQL = sSQL
		SendErrorEmailToMark ErrorSubject, ErrorSQL
		sFirstName = ""
		sLastName = ""
...
' IF sMemberID="000001151" THEN
' 		response.write("<br><br><br>sPassword = "&sPassword)
' 		response.write("<br>SearchSuccess = "&SearchSuccess)
' 		response.write("<br>LCASE(sMemberPW) = "&LCASE(sMemberPW))
' 		response.write("<br>adminmenulevel = "&adminmenulevel)
' END IF
...
IF adminmenulevel>= 20 THEN
		ErrorSubject="INFORMATION - Line 357 - Search-MemberHQ - sPassword ="&sPassword
		ErrorSQL = "adminmenulevel = "&adminmenulevel
		'SendErrorEmailToMark ErrorSubject, ErrorSQL
...
ELSEIF TRIM(sTourAdminPW)<>"" AND ( sUserAdminPW=LCASE(sTourAdminPW) OR LCASE(sPassword)=LCASE(sTourAdminPW) ) THEN
		ErrorSubject="INFORMATION - Line 366 - Search-MemberHQ - sPassword ="&sPassword
		ErrorSQL = "TRIM(sTourAdminPW) = "&TRIM(sTourAdminPW)&" - sUserAdminPW = "&sUserAdminPW 
		'SendErrorEmailToMark ErrorSubject, ErrorSQL
...
ELSEIF sPassword<>"" AND SearchSuccess="Y" AND LCASE(sMemberPW)=LCASE(sPassword) THEN
		ErrorSubject="INFORMATION - Line 375 - Search-MemberHQ - sPassword ="&sPassword
		ErrorSQL = "TRIM(sMemberPW) = "&TRIM(sMemberPW)
		'SendErrorEmailToMark ErrorSubject, ErrorSQL
		...
		<TABLE class="innertable" BORDER="4" ALIGN="CENTER" width="60%" >
	  	...
	  	<TR>
   			...
   			<TABLE BORDER="0" ALIGN="CENTER" CELLPADDING="3" CELLSPACING="0" BGCOLOR="#FFFFFF" width="90%" >
		    	...
		    	IF sPassword <> "" AND sPassword<>"manual" AND SearchSuccess="Y" AND LCASE(sPassword) <> LCASE(sPassword) THEN  
			    		%>
	  	       	<tr>	
...
IF TRIM(sMemberID) <> "" THEN rsPW.close
	    		...
	    		<td align ="left" style="border-style:none;"> 
	    			<%	
						IF PWreq = "yes" THEN
		  					...
		  					%><form action="/rankings/<%=ThisFileName%>?FormStatus=getpw" method="post"><%
						ELSE
								%><form action="/rankings/<%=ThisFileName%>?FormStatus=confirmed&m=test" method="post"><%
						END IF  
						%>
							<input type="hidden" name="sMemberID" value="<%=sMemberID%>">
							<input type="hidden" name="sSendingPage" value="<%=sSendingPage%>">
...
BannerHeadline = "Search for Member"
IF LCASE(LEFT(sSendingPage,27)) = "/rankings/register_emailmes" THEN BannerHeadline = "Please Identify Yourself" 
...
<TABLE ALIGN="CENTER" class="innertable" width="70%" >
  <tr>
  	<th align=center>
			<font face=<% =font1 %> size="4" Color="<%=TextColor5%>"><b><% =BannerHeadline %></b></font>
      <br>
    </th>
  </tr>  
  <tr>
