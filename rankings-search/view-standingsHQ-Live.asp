...
Dim sMemberID, sPersonID, sFullName, sEventName
...
Dim ThisFileName
ThisFileName="View-StandingsHQ-Live.asp"
...
ELSE
	...
	ELSE 		' --- Displays page ---
		...
		IF tRCU = "Y" THEN   ' --- Calc underway - Tell them to try again later
...
'	First conditional posts bold disclaimer if current incomplete ski year selected.
	...
	IF sRunByWhat<>"NCWSA" AND sRunByWhat<>"NSL" THEN %>
		...
		<Th ALIGN="Center" ><font size=<% =fontsize2 %> COLOR="#FFFFFF"><b><br>Percentile</b></FONT></Th>
	<% END IF 
...
ELSE   
   ...
   ELSEIF tDefaultYear = 0 and (DivSelected = "OM" or DivSelected = "OW" or DivSelected = "MM") THEN
      tEliteStat = "None"
...
'    sSQL = sSQL & " RQ.QfyStatus, RQ.QfyStatusTEMP, RQ.TourID AS RQTourID, Coalesce(LT.HomoType,'-') AS HomoType, LT.Status AS TourStatus"
    ...
    sSQL = sSQL & " FROM "&RankTableName&" as RT JOIN "&MemberLiveTableName&" as MT on cast(right(RT.memberid,8) as integer) = MT.personid "
...
<tr>
  ...
  <td colspan=2>	
	...
	' --- AWSA does not display Current Ski Year Rankings -- removed 07/20/2010 DJC
	'	IF sRunByWhat <> "NSL" AND sRunByWhat <> "NCWSA" THEN
...
<tr>
  ...
  IF 1=1 OR sRunByWhat="NSL" OR sRunByWhat="NCWSA" THEN %>
		<td>&nbsp;</td>
		<td>&nbsp;</td><%
...
<tr>
  ...
  <font size=<% =fontsize2 %> face=<% =font1 %> COLOR=<%=TitleCOLOR%>><b>
  ...
  ELSE 
   	IF TRIM(session("SkiYear"))=1 THEN %>
		<td align="right">
		  <font size=<% =fontsize2 %> face=<% =font1 %> COLOR=<%=TitleCOLOR%>><b>Regl/Natl Place:</b> </font>
		</td>

		<td align="left">
		  <% ' --- This may need to be dynamic by Div/Event --- %>
    		  <font size=<% =fontsize2 %> face=<% =font1 %> COLOR="red"><b>&nbsp;Top 5</b></font>
		</td>
		<td>&nbsp;</td><%
		ELSE %>
			<td>&nbsp;</td>
...
<% IF DivSelected = "" THEN %>
   <td align="center"><input type=submit style="width:10em" value="Display Rankings"
   	  title="Display Rankings for the selection parameters above/left"></td>
<% ELSE %>
   <td align="center"><input type=submit style="width:10em" value="Update Display"
   	  title="Display a revised Rankings page after you have &#13;changed the selection parameters above/left"></td>
<% END IF %>

   <td align="center"><input type=submit style="width:10em" name="SingleRanking" value="Find My Rankings"
   	  title="Look up my Rankings in all Events"></td><%
...
sSQL = sSQL + " JOIN "&MemberLiveTableName&" AS MT ON cast(right(RT.memberid,8) as integer) = MT.personid"
...
<TR>
  <Th>
    <center><font size=<% =fontsize3 %> COLOR="<%=TextCOLOR1%>"><b> National Rankings For</font>
      <font size=4 COLOR="white"><br><%=rsRankList("FirstName")%>&nbsp;<%=rsRankList("LastName")%></b></FONT>
      <font size=2 COLOR="<%=TextCOLOR1%>"><br><b><%=rsRankList("City")%>, <%=rsRankList("State")%></b></FONT>
    </center>
  </th>
</TR>
<TR>
  <td>  
    ...
    <TR>
    	<Th ALIGN="Center" Width=20% vAlign="center" bgCOLOR="<%=HeadCOLOR1%>"><font size=<% =fontsize2 %> COLOR="#000000"><b>National<br>Rank</b></FONT></Th>
    	<Th ALIGN="Center" Width=30% vAlign="center" bgCOLOR="<%=HeadCOLOR1%>"><font size=<% =fontsize2 %>  COLOR="#000000"><b>Division<br>and Event</b></FONT></Th> 	
    	<Th ALIGN="Center" Width=30% vAlign="center" bgCOLOR="<%=HeadCOLOR1%>"><font size=<% =fontsize2 %>  COLOR="#000000"><b>Ranking<br>Score</b></FONT></Th> 	
    </TR><%
	...
	%>
    	...
    	<TR>
	  <TD ALIGN="Center" Width=9% vAlign="top" bgCOLOR="<%=TableCOLOR1%>"><font size=<% =fontsize2 %> COLOR="#000000"><b><%=rsRankList("RankNum")%></b></FONT></TD>
	  ...
	  <TD ALIGN="Center" vAlign="top" bgCOLOR="<%=TableCOLOR1%>"><font size=<% =fontsize2 %>  COLOR="#000000"><b><%=sRankScore%></b></FONT></TD> 	
	</TR><%
        rsRankList.MoveNext	
    LOOP %>
...
<TABLE class="noborder" align="center" border=0 width=70%>
<tr>
  <td colspan=3 align="center">
	<font size=<% =fontsize2 %> face=<% =font3 %> COLOR="#000000"><b>&nbsp;<br>National Ranks are based on all USA skiers in this Division and Event.<br> Regional or Filtered rankings may be different.</b></font>
	<br><br>
	<font size=<% =fontsize2 %> face=<% =font3 %> COLOR="#000000"><b>Click on any Division / Event Link above to display complete ranking details in <br>that specific Division and Event</b></font>
	<br><br>
  </td>
</tr>
...
<tr>
  ...
  <td align="center">
	<input type=submit name="NewMember" value="New Member" 
		title="Select another member and Display their Personal Rankings"></center>
...
ELSE  %>
	...
	<TR>
	  <TD Colspan=2 align=center BGCOLOR="<%=HQSiteCOLOR2%>">
...
'END IF
	...
	IF TRIM(rs("TourStatus"))<>"X" AND ( TRIM(rs("HomoType"))="A" OR TRIM(tRegion)=RIGHT(LEFT(rs("RQTourID"),3),1) ) THEN  
		%>
		<a href="/rankings/MemberQualifications.asp?sMemberID=<%=tMemberID%>&sTourID=<%=TRIM(rs("RQTourID"))%>" title="Check details of Qualifications for this Member" target="_blank">
	      	<font size=<% =fontsize2 %>><%=rs("QfyStatus")%></FONT></a><%
'response.write("<br>1")
 	ELSEIF TRIM(rs("TourStatus"))<>"X" AND TRIM(tRegion)<>RIGHT(LEFT(rs("RQTourID"),3),1) THEN %>
		<a href="/rankings/MemberQualifications.asp?sMemberID=<%=tMemberID%>&sTourID=<%=TRIM(rs("RQTourID"))%>" title="Check details Qualifications for this Member.  This Member's HOME REGION is <%=tRegion%>" target="_blank">
	      	<font size=<% =fontsize2 %>>OOR</FONT></a><%
'response.write("<br>2")
 	ELSEIF TRIM(rs("TourStatus"))<>"X" AND LCASE(TRIM(sLeagueSelected))="none" AND LCASE(TRIM(sLeagueSelected))<>"" THEN %>
		<a href="/rankings/MemberQualifications.asp?sMemberID=<%=tMemberID%>" title="Check details Qualifications for various Tournament/Leagues for this Member." target="_blank">
	      	<font size=<% =fontsize2 %>>View</FONT></a><%
'response.write("<br>3")
	ELSE 
'response.write("<br>4")
		%>
	      	<font size=<% =fontsize2 %>>---</FONT></a><%
   ...
   IF Session("adminmenulevel")<10 THEN
	...
	<TABLE class="innertable" ALIGN=CENTER WIDTH=500 BORDER=0>
		...
		<th colspan=3 align="center"><font size="<%=fontsize1%>" COLOR="<%=TextCOLOR5%>">&nbsp;&nbsp;Percentiles and COA Scores For&nbsp; <%=DivSelected%>&nbsp; <%=sEventName%></font></th>
         ...
         <tr><td align="center" style="background-COLOR:<%=sCOLOR09%>"><font size="<%=fontsize1%>">Level 9</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp;&nbsp;( 93-100 ) Percentiles </font></td><td align="center"><font size="<%=fontsize1%>">&nbsp; COA Score:&nbsp; <%=COALevel9%>
	  	  		<% IF EventSelected = "O" THEN %>
				<font COLOR="red"> **</font>
			   <% END IF %>
         </font></td></tr>
        	<tr><td align="center" style="background-COLOR:<%=sCOLOR08%>"><font size="<%=fontsize1%>">Level 8</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp;&nbsp;( <%=tPerc8%>-92 ) Percentiles </font></td><td align="center"><font size="<%=fontsize1%>">&nbsp; COA Score:&nbsp; <%=COALevel8%></font></td></tr>
	      <tr><td align="center" style="background-COLOR:<%=sCOLOR07%>"><font size="<%=fontsize1%>">Level 7</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp;&nbsp;( <%=tPerc7%>-<%=(tPerc8)-1%> ) Percentiles</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp; COA Score:&nbsp; <%=COALevel7%></font></td></tr>
        	<tr><td align="center" style="background-COLOR:<%=sCOLOR06%>"><font size="<%=fontsize1%>">Level 6</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp;&nbsp;( <%=tPerc6%>-<%=(tPerc7)-1%> ) Percentiles</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp; COA Score:&nbsp; <%=COALevel6%></font></td></tr>
	      <tr><td align="center" style="background-COLOR:<%=sCOLOR05%>"><font size="<%=fontsize1%>">Level 5</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp;&nbsp;( <%=tPerc5%>-<%=(tPerc6)-1%> ) Percentiles</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp; COA Score:&nbsp; <%=COALevel5%></font></td></tr>
        	<tr><td align="center" style="background-COLOR:<%=sCOLOR04%>"><font size="<%=fontsize1%>">Level 4</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp;&nbsp;( <%=tPerc4%>-<%=(tPerc5)-1%> ) Percentiles</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp; COA Score:&nbsp; <%=COALevel4%></font></td></tr>
	      <tr><td align="center" style="background-COLOR:<%=sCOLOR03%>"><font size="<%=fontsize1%>">Level 3</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp;&nbsp;( <%=tPerc3%>-<%=(tPerc4)-1%> ) Percentiles</font></td><td align="center"><font size="<%=fontsize1%>">&nbsp; All Others</font></td></tr><%
