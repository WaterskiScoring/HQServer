...
Dim MainImage, sl, RecCnt, DefineLevelColor, sShowSQL, pvar
...
ThisFileName="view-grRanking.asp"
...
%>
<br><br><br><br>
<center><font size=3>This module is no longer active</font></center>
<%
response.end
...
' --- pvar controls which list ---
pvar = LCASE(TRIM(Request("pvar")))
IF pvar="national" OR pvar="junior" OR pvar="ncwsa" THEN
	response.redirect("/rankings/view-standingsHQ.asp?pvar="&pvar&" ")
END IF

IF pvar="" THEN pvar="grassroots"

' ---
IF LCASE(action)="Return to Menu" THEN
...
PageSubTitle=sSkiYearName&" Rankings"
...
IF TRIM(LCASE(WhatHeadFoot))="rs" THEN 
	...
	WhatCSSTable="droptable"
ELSE
	' --- In Tools_GRSite.asp
	WriteGRHeader
	WhatCSSTable="GRTable2"
END IF
...
<br>
<center>
<font color="<%=TextColor5%>" size="3"><b>Select Event and press Display Report button.<br><br> It may take several seconds to display</b></font></center><%
...
<TABLE class="<%=WhatCSSTable%>" Align=center WIDTH=725px height=200 background="<%=MainImage%>">
  <TR>
	<td colspan=2>
