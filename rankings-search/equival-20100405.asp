...
sSQL = sSQL & " when RS.Div = 'S3' then 'M5' when RS.Div = 'L1' then 'W3'"
sSQL = sSQL & " when RS.Div = 'L2' then 'W4' when RS.Div = 'L3' then 'W5'" 
...
sSQL = sSQL & EquivScoresTableName & " Where Div in ('M3','M4','M5','M6','MM') and SkiYearID = " & sProcessingYear
sSQL = sSQL & " Group by 	MemberID, TourID, Event, Round, Class, DivOrig, Place, SkiYearID, Team, PrioClass, TeamStat) as XS, "
sSQL = sSQL & DivisionsTableName & " as DO, " & DivisionsTableName & " as DE Where DO.Div = XS.DivBase"
sSQL = sSQL & " and DO.SkiYearID = XS.SkiYearID and	DE.Div = XS.DivElite and DE.SkiYearID = XS.SkiYearID) as ES, "
sSQL = sSQL & DivisionsTableName & " as DE WHERE DE.Div = ES.Div and DE.SkiYearID = ES.SkiYearID"
...
sSQL = sSQL & " WHERE ES.DivType <> 'D' and ES.SkiYearID = " & sProcessingYear 
...
sSQL = "UPDATE RT SET AWSA_Rat = CT.Event + '9' FROM " & RankTableName & " as RT, "
sSQL = sSQL & RankTableName & " as ET, " & CutOffTableName & " as CT, " & MemberTableName & " as MT" 
sSQL = sSQL & " WHERE ET.MemberID = RT.MemberID and RT.MemberID = MT.PersonIDWithCheckDigit"
sSQL = sSQL & " and ET.Event = CT.Event and CT.Event = RT.Event and RT.Event = 'O'"
sSQL = sSQL & " and ET.SkiYearID = CT.SkiYearID  and CT.SkiYearID = RT.SkiYearID"
sSQL = sSQL & " and RT.SkiYearID = " & sProcessingYear & " and ET.Div = CT.Div and CT.Div = CASE"
sSQL = sSQL & " When RT.Div in ('B2','B3','M1','M2','OM') then 'EM'"
sSQL = sSQL & " When RT.Div in ('G2','G3','W1','W2','OW') then 'EW'"
sSQL = sSQL & " When RT.Div in ('M3','M4','M5','M6','MM') then 'SM' else '??' end"
sSQL = sSQL & " and CT.COA9 <= ET.RankScore and UPPER(MT.FederationCode) = 'USA'"
