...
'response.end
  ...
  CASE "addrec"
    	...
    	WriteHeaders "Add Record:  "&ThisFieldID&" Code = "&sThisFieldID&" FOR SptsGrpID = '"&Session("sSptsGrpID")&"' IN "&ThisTempTableName
...
'response.write(len(sThisFieldID) <> LengthofID)%><br><%
		...
		%><br><H2><center><font color="red">You must Specify a <%=LengthofID%>-character Code (ID) for the<BR> <%=KeyFieldName%> that you wish to have added, in the box below.<BR></font></center></H2><% 	
    		...
    		Listrecords
	ELSE    	
		ChooseSQL "SELECT * FROM " & ThisTempTableName & " WHERE (SptsGrpID='"&Session("sSptsGrpID")&"' AND "&ThisFieldID&"='"&sThisFieldID&"')"
		IF rs.EOF THEN TempEOF = "Y" ELSE TempEOF = "N"
		rs.close: set rs = nothing

		IF TempEOF = "N" THEN
			%><br><H2><center><font color="red">The Code (ID) that you requested to be<br>added already exists.<br><BR></font></center></H2><%
	    		Listrecords
		ELSE
			AddRecord
			ShowEditor
  ...
  CASE "editrec"
    WriteHeaders "Edit "&KeyFieldName&" Record:  "&KeyFieldName&" Code = "&sThisFieldID&" for Sports Discipline = "&session("sSptsGrpID")&" in " & ThisTempTableName
    ShowEditor

  CASE "saverec"
    WriteHeaders "Record Saved :  "&KeyFieldName&" Code = "&sThisFieldID&" for Sports Discipline = "&session("sSptsGrpID")&" in " & ThisTempTableName
    SaveRec

  CASE "delrec"
    WriteHeaders "Delete "&KeyFieldName&" Record:  "&ThisFieldID&" Code = "&sThisFieldID&" for Sports Discipline = "&session("sSptsGrpID")&" from " & ThisTempTableName
    DeleteRec
...
'response.end
  ...
  ChoosePagesSQL sSQL,currentPage, 150



ELSE

  sSQL = "SELECT * FROM "&ThisTempTableName&" AS MT WHERE 1=1"
...
ChooseSQL "SELECT * FROM "&ThisTempTableName&" WHERE (teamID='"&sThisFieldID&"' AND SptsGrpID='"&session("sSptsGrpID")&"')"
...
ChooseSQL "SELECT * FROM "&ThisTempTableName&" WHERE ("&ThisFieldID&"='"&sThisFieldID&"' AND SptsGrpID='"&session("sSptsGrpID")&"')"
...
rs.close
set rs = nothing
sSQL = sSQL + " WHERE "&ThisFieldID&"='"&sThisFieldID&"' AND SptsGrpID='"&session("sSptsGrpID")&"'"
OpenCon
...
' Then the Editing window will be presented -- by mainline CASE.
 		...
 		sSQL = "INSERT INTO "&ThisTempTableName&" ("&ThisFieldID&", SptsGrpID) VALUES ('"&sThisFieldID&"', '"&Session("sSptsGrpID")&"')"
		OpenCon
		con.execute(sSQL)
		Closecon
...
IF LCASE(Request("confirm")) = "yes" THEN
    'delete the record'
    'WriteLog(date() &"  "& time() &"  "&KeyFieldName&" Table Record " & rs("&ThisFieldID&") & " for (" & rs("&MyKeyFieldName2&") & ") has been deleted.")
...
'response.write(isrecordsetempty = false)
	...
	OpenCon
	sSQL = "DELETE FROM "&ThisTempTableName&" WHERE "&ThisFieldID&"='"&sThisFieldID&"' AND SptsGrpID='"&session("sSptsGrpID")&"'"
...
IF h > 21 THEN
  ...
  FOR i = 10 TO 1 step -1
    IF currentpage - i > 0 THEN
      	Response.Write(" <a href=" & chr(34) & ThisPage & "?currentpage=" &  currentpage - i  & "&action1=" & sAction & "&SortField="&sSortField& chr(34) &">" & currentpage - i & "</a>")
    END IF
  NEXT
  ...
  FOR i = 1 TO 10
   	IF currentpage + i <= h THEN
      		Response.Write(" <a href=" & chr(34) & ThisPage & "?currentpage=" &  currentpage + i  & "&action2=" & sAction & "&SortField="&sSortField& chr(34) &">" & currentpage + i & "</a>")
	END IF
  NEXT
...
ELSE
  FOR i = 1 TO h
    Response.Write(" <a href=" & chr(34) & ThisPage & "?currentpage=" &i& "&action=" &sAction& "&SortField=" &sSortField& chr(34) & ">" &i& "</a>")
  next
