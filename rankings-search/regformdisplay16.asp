...
function ShowClassHelp()
	{
 		var classHelpWindow = window.open("","Preview","width=325px, height=500px, scrollbars=yes, resizable=yes, status=0");

    classHelpWindow.document.open();
    classHelpWindow.document.writeln('<HTML><HEAD><TITLE>Homologation Class Help</TITLE></HEAD>');
    classHelpWindow.document.writeln('<BODY style="font-family: Arial, Helvetica, sans-serif; text-align:left; font-size:10pt;">');

    classHelpWindow.document.writeln('<div style="width:100%; margin:40px 0px 0px 0px;"><h3>WHAT DO CLASSES MEAN?</h3></div>');  
    classHelpWindow.document.writeln('<div style="width:100%; margin:20px 0px 0px 0px;"><b>CLASS R:</b> Scores may be used to set World Records. Scores also qualify to be included on the World Ranking and AWSA Ranking lists.</div>');
    classHelpWindow.document.writeln('<div style="width:100%; margin:20px 0px 0px 0px;"><b>CLASS L:</b> Scores qualify to be included on the World Ranking and AWSA Ranking lists.</div>');   
    classHelpWindow.document.writeln('<div style="width:100%; margin:20px 0px 0px 0px;"><b>CLASS E:</b> Scores may be used to set US National Records and will be included on the AWSA Ranking list.</div>');   
    classHelpWindow.document.writeln('<div style="width:100%; margin:20px 0px 0px 0px;"><b>CLASS C:</b> Standard tournament classification.  Scores qualify for AWSA Ranking List.</div>');     
    classHelpWindow.document.writeln('<div style="width:100%; margin:20px 0px 0px 0px;"><b>GRASSROOTS:</b> Scores less than Level 6 qualify for AWSA Ranking List</div>');        
    classHelpWindow.document.writeln('<br><div style="width:100%; margin:10px 0px 0px 0px;">All classes listed in the drop down may not be offered for all events/divisions.  This is up to the discretion of the tournament organizer and may change depending on number of entries received.</div>');
    
    classHelpWindow.document.writeln('<div style="width:100%; margin:10px 0px 0px 0px; text-align:center;"><A HREF="javascript:window.close()">Close Help</A><BR></div>');

		// classHelpWindow.document.writeln(document.SampleForm.MessageText.value);
    classHelpWindow.document.writeln('</BODY></HTML>');
    classHelpWindow.document.close();
	}
  ...
  <div id="RegPanel1" class="tour_div" style="padding-top:10px; display:<% IF nav=1 THEN response.write("block") ELSE response.write("none") END IF %>;">
      ...
      <span class="tourgendataC1" style="width:555px;"><%= GTComments %>&nbsp;</span>
		  ...
		  <div class="feerowdiv">
	   		...
	   		<span id="sClassFeeG2" class="feeamt"><%= sClassFeeG2Text %></span>
	   		<span id="sClassFeeG3" class="feeamt"><%= sClassFeeG3Text %></span>
...
ELSEIF LEFT(Session("sExpirationStatusText"),2)<>"OK" THEN 
		...
		UpgradeRemedyText = "Your Membership <b><u>expires</u></b> before the start date of tournament and must be renewed before you can participate.  To renew your membership, press the <b>Member Renewal</b> button below.  <br><br>Once you complete your membership renewal, return to this page by restarting the registration process.  If you are entering the same day you upgraded your membership, press <b>Verify Renewal</b> to continue with online registration."
END IF
...
rk=1
IF sMemberID="000001151" AND rk=2 THEN
		' response.write("<\div><div>")
		' response.write("<br>")
		response.write("<span class=eventline><font style='color:black;'>")
		response.write("<br><br>RFD16 1082=")
		response.write(sFeeRounds(EvtNo))
...
END IF	' --- For Admin Override section ---
	...
	<span class="spanbuttons" style="margin-top:10px; padding-bottom:10px; width:98%;">
		...
		<input type="submit" style="width:9em; position:absolute; left:490px;" value="FAQ"  title="Frequently asked questions" onclick="javascript:ShowClassHelp();">
	</span>	
<br><br>
...
END IF
  ...
  <div id="RegPanel4" class="tour_div" style="padding-top:10px; display:<% IF nav=4 THEN response.write("block") ELSE response.write("none") END IF %>;">
  			...
  			' ---------------------------------
				...
				IF sEntryType="FAM" THEN
		   			...
		   			IF MainStatus<>"Verify" OR TotQualifyingFamMemb>0 THEN 
		   					%>
		   					<br>
			  				<div class="olrsecheading" style="border:0px solid; width:100%; margin-top:10px; color:red;">IMPORTANT INFORMATION ABOUT FAMILY ENTRIES</div>
								<div class="feerowdiv" style="border:0px solid black; width:100%; height:50px; margin-top:5px;">
			    				<%
	  ...
	  IF sJrDiscPerc <> 0 AND sMembAge < 18 AND sEntryFee > 0 THEN 
		...
		LateFeeDaysText = ""
		IF sTLFPerDay=true THEN LateFeeDaysText = "("&sLateDays&"d)"
...
RunOldWaiverHeader="N"
IF RunOldWaiverHeader="Y" THEN 	
...
ELSE	
 	...
 	%> 
  <div style="width:100%" class="<% IF nav=5 THEN response.write("accordionHeaderSelected") ELSE response.write("accordionHeader") END IF %>">
		<span style="text-align:left; width:30%;">
			<a title="Please use Navigation Buttons">STEP 5 - Waiver</a>
		</span>
	</div>	
	<% 
...
ELSE
		Dim sThisName
		sThisName = TRIM(sFirstName) &" "& TRIM(sLastName)
		' --- If the total fees less banquet tickets and AWSEF are greater than zero ---
		IF RegFeeAmount>0 THEN
				ItemNo=ItemNo+1
				Item_name(ItemNo)="Registration for "& sThisName &" - Member # "&sMemberID&" at "&sTourName
				Quantity(ItemNo)="1"
