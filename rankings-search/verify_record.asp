...
%>
          ...
          <h2> All of the exceptions in <br>
          	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=Request("file")%> <br>
          	have now been corrected.</h2><p>

<%

          sTourID = ucase(mid(request("file"),12,7))
          IF instr("LRPAB",right(sTourID,1)) > 0 THEN
            Session("TourID") = sTourID
       		
       		%>
        	    
        	 <h2>Click the button below to produce the IWWF Rankings<br>Export of all Class L/R scores, which will be sent to IWWF ...</h2>
        	 <br><br>
        ...
        <% ELSE %>
        	 ...
        	 <h2>Click the button below to return to the Main Menu.</h2>
        	 <br><br>
        	 
           <form method=post action="DefaultHQ.asp?process=uploadany" method="post">
              <input type="submit" style="width:13em" value="Exceptions Done"
...
'    sSQL = "Select top 1 * from "&MemberTableName&" where PersonIDwithCheckDigit = '" & right(left(datain,12),9) & "'"
     sSQL = "Select top 1 * from "&MemberShortTableName&" where PersonID = '" & right(left(datain,12),8) & "'"
     rs.open sSQL, sConnectionToTRATable
     if rs.eof then
