...
ThisFileName="view-scoresHQ.asp"
...
IF sRunByWhat="ByMember" THEN Action="ByMember"
IF sRunByWhat="FoundMember" THEN Action="ByMember"
...
' ----------------------------
		...
		'response.write("<br><br>412 Found = ")
    ...
    ' -------------
		IF sTourClass <> "" THEN
		  ...
		  ELSEIF sTourClass = "FNI" or Session("NSL") = "1" THEN
		...
		IF StateSelected <> "" THEN
		  	sSQL = sSQL + " AND lower(ST.TState) = '"&sqlclean(LCASE(StateSelected))&"'"
		END IF
	...
	<tr>
		...
		DO WHILE Not rs.EOF 
						...
						<tr>
							...
							<TD ALIGN="center" vAlign="top" bgcolor="#C0C0C0">
								<font size=<%=fontsize2%> face="<%=font2%>" color="<%=Textcolor1%>">
									<a href="/rankings/<%=ThisFileName%>?tour_id=<%= rs("TSanction") %>&sTourSportsGroup=<%= sTourSportsGroup %>&action=tourscores">
										<%= rs("TName") %>
									</a>
								</font
							</TD>
				      ...
				      IF Adminmenulevel>=19 THEN 
									...
									<TD ALIGN="Center" vAlign="top" bgcolor="#C0C0C0">
										<font size=<%=fontsize2%> face="<%=font2%>" color="<%=Textcolor1%>">
											<a href="/rankings/<%=ThisFileName%>?tour_id=<%=rs("TSanction")%>&sTourSportsGroup=<%=sTourSportsGroup%>&pvar=ByTour">
												<%=rs("TStatus")%>
											</a>
										</font>
									</TD>
	  ...
	  <TD ALIGN="Center" vAlign="top">
		...
		IF EventSelected <> "O" THEN
		   IF Session("NSL") <> "1" THEN 
			...
			IF left(EventSelected,1) = "S" THEN %>
				<TD ALIGN="Center" vAlign="top"><font size=<%=fontsize2%> face="<%=font2%>" color="<%=Textcolor1%>">&nbsp;<%=ABS(rs("AltScore"))%></FONT></TD><%
			END IF
