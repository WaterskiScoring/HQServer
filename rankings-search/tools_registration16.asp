...
Dim sSiteLatitude, sSiteLongitude, sSiteAddress, sSiteZip
...
Dim sTDirName, sTSponsor, sTSite, sTSiteID, sTOpenClosed, sTTowBoatClosed, sTEntryLimit, sTStatus, sTDeleted
...
' --- AWSA and NCW uses beginning in 2016 --- 
Dim sClassFeeG1, sClassFeeG2, sClassFeeG3
Dim sClassFeeC1, sClassFeeC2, sClassFeeC3
Dim sClassFeeE1, sClassFeeE2, sClassFeeE3
Dim sClassFeeL1, sClassFeeL2, sClassFeeL3
Dim sClassFeeR1, sClassFeeR2, sClassFeeR3
Dim sClassFeeCash1, sClassFeeCash2, sClassFeeCash3


' --- Kneeboard uses ---
Dim sClassFeeQ1, sClassFeeQ2, sClassFeeQ3, sClassFeeT1, sClassFeeT2, sClassFeeT3

	' --- Wakeboard uses ---
Dim sClassFeeW1, sClassFeeW2, sClassFeeW3
...
Dim FormErrorDisplayStatus, MultipleClassDisplayStatus
...
' --- www.usawaterski.org/sanctions/sanctions/mark.aspx  as your starting point.
' --- It will get you into the sanction system using the new forms and bypassing various checks like club membership 
' ---    and restrictions on when new sanctions can be created. 

' --- If you choose to create a new sanction put it in January and use Twin Lakes Water Ski Club as the LOC and 
' ---    Twin Lakes Waterski Park as the site.  
' ---    I have notified the Eastern region rep to expect bogus emails about the test tournaments and set up a bogus pay pal account.

' --- Using the edit function you can change events and fees to create any test setup you choose.  
' --- You can also see what the LOC sees when he is setting up OLR.  It displays admin code, pay pal acct, club code, etc.


' ---------------------------------------------------------
' ---------------------------------------------------------
' --- I have created two sanctions for you to play with
...
ELSE	' --- Found some valid tour and function - END IF at bottom of SUB ---
		sValidTour = true
...
'	END IF
    ...
    IF NOT rsTSetup.eof THEN	' - END IF at bottom of SUB ---
				

	' --- Top of variable definitions NOT INDENTED per IF indent --
	...
	sSiteLatitude = rsTSetUp("SiteLat")
	sSiteLongitude = rsTSetUp("SiteLong")
	'sSiteAddress = rsTSetUp("TSiteAddress")
	sSiteZip = rsTSetUp("SiteZip")
	...
	' -------------------------------------------------------------------------------------------------
  			...
  			ELSE
  					sTLateDate_Adjusted = FormatDateTime(sTLateDate) 
  			END IF 
	...
	sTTowBoatClosed=rsTSetUp("TTowBoatClosed")
	...
	SClassC=rsTSetUp("SClassC")		' --- The following fields are INTEGER format
	...
	' --------------------------------------
	' --- PayPal and payment information ---
	' --------------------------------------
	...
	' ------------------------------------------------------------------------------
	' --- Determines if payment processor goes to PayPal or USA Merchant Account ---
	' ------------------------------------------------------------------------------
	...
	TestPayPal="N"
	IF (sMemberID="000001151" OR sMemberID="1151") AND TestPayPal="Y" THEN
			sHQAccount=false
	...
	' -----------------------------------------------------
	' --- Controls whether OLR button is visible or not ---
	' -----------------------------------------------------
	...
	' --- sTEventSlalom, sTEventTrick, sTEventJump determine whether or not that event is defined in Tools_Include.asp
	...
	TotNumEvents = 0
	IF SClassC>0 OR SClassE>0 OR SClassL>0 OR SClassR>0 OR SClassCash>0 OR SClassX>0 THEN TotNumEvents = TotNumEvents + 1
	IF TClassC>0 OR TClassE>0 OR TClassL>0 OR TClassR>0 OR TClassCash>0 OR TClassX>0 THEN TotNumEvents = TotNumEvents + 1
	IF JClassC>0 OR JClassE>0 OR JClassL>0 OR JClassR>0 OR JClassCash>0 OR JClassX>0 THEN TotNumEvents = TotNumEvents + 1
				
	IF BSClassC>0 OR BSClassL>0 OR BSClassR>0 THEN TotNumEvents = TotNumEvents + 1
	IF BTClassC>0 OR BTClassL>0 OR BTClassR>0 THEN TotNumEvents = TotNumEvents + 1
	IF BJClassC>0 OR BJClassL>0 OR BJClassR>0 THEN TotNumEvents = TotNumEvents + 1

	IF USClassC>0 THEN TotNumEvents = TotNumEvents + 1
	IF UTClassC>0 THEN TotNumEvents = TotNumEvents + 1
	IF UJClassC>0 THEN TotNumEvents = TotNumEvents + 1

	IF KSClassQ>0 OR KSClassT>0 THEN TotNumEvents = TotNumEvents + 1
	IF KTClassQ>0 OR KTClassT>0 THEN TotNumEvents = TotNumEvents + 1
	IF KlClassQ>0 OR KlClassT>0 THEN TotNumEvents = TotNumEvents + 1
	IF KrClassQ>0 OR KrClassT>0 THEN TotNumEvents = TotNumEvents + 1

	IF WWakeW>0 THEN TotNumEvents = TotNumEvents + 1
	IF WSkateW>0 THEN TotNumEvents = TotNumEvents + 1
	IF WSurfW>0 THEN TotNumEvents = TotNumEvents + 1
	...
	' --- New FEE fields starting Ski Year 2016 - Integer and default to 0 (zero) - Use for awsa, ncwsa, abc ---
	sClassFeeC1 = rsTSetUp("ClassFeeC1")
	sClassFeeC2 = rsTSetUp("ClassFeeC2")
	sClassFeeC3 = rsTSetUp("ClassFeeC3")
	sClassFeeE1 = rsTSetUp("ClassFeeE1")
	sClassFeeE2 = rsTSetUp("ClassFeeE2")
	sClassFeeE3 = rsTSetUp("ClassFeeE3")
	sClassFeeL1 = rsTSetUp("ClassFeeL1")
	sClassFeeL2 = rsTSetUp("ClassFeeL2")
	sClassFeeL3 = rsTSetUp("ClassFeeL3")
	sClassFeeR1 = rsTSetUp("ClassFeeR1")
	sClassFeeR2 = rsTSetUp("ClassFeeR2")
	sClassFeeR3 = rsTSetUp("ClassFeeR3")
	sClassFeeCash1 = rsTSetUp("ClassFeeCash1")
	sClassFeeCash2 = rsTSetUp("ClassFeeCash2")
	sClassFeeCash3 = rsTSetUp("ClassFeeCash3")

	' --- Kneeboard uses ---
	sClassFeeQ1 = rsTSetUp("ClassFeeQ1")
	sClassFeeQ2 = rsTSetUp("ClassFeeQ2")
	sClassFeeQ3 = rsTSetUp("ClassFeeQ3")
	sClassFeeT1 = rsTSetUp("ClassFeeT1")
	sClassFeeT2 = rsTSetUp("ClassFeeT2")
	sClassFeeT3 = rsTSetUp("ClassFeeT3")

	' --- Wakeboard uses ---
	sClassFeeW1 = rsTSetUp("ClassFeeW1")
	sClassFeeW2 = rsTSetUp("ClassFeeW2")
	sClassFeeW3 = rsTSetUp("ClassFeeW3")
	...
	' --- Changed to be consistent with other classes ---
	sClassFeeG1 = rsTSetUp("GRFee_1")
	sClassFeeG2 = rsTSetUp("GRFee_2")
	sClassFeeG3 = rsTSetUp("GRFee_3")
	
	' --- Override since sanctions cannot handle it --
	IF LEFT(sTourID,6)="19E011" THEN 
			sClassFeeG1 = 35
			sClassFeeG2 = 2
			sClassFeeG3 = 2
	END IF
	...
	IF TRIM(sOF10Desc)<>"" THEN TotNumOptItems=TotNumOptItems+1
		...
		END IF	' --- From Original condition at top NOT EOF ---

END IF 	' --- From Original condition at top ---
...
' **********   TEST SETTINGS FOR 2016 RELEASE  *******


MarkTestingNew=2
IF MarkTestingNew=1 THEN
		...
		' --- TEST REMOVE ---
		'Session("sExpirationStatusText")="OK"
		'Session("sMembCanSkiText")="OK"
		...
		sJrDiscPerc = 10 
		sSrDiscPerc = 20 
		sMembAge = 60 	' -- Age LT 28 Junior or GT 59 for Senior

		' --- Criteria for application of SR discount
		' cdbl(sSrDiscPerc) <> 0 AND sMembAge > 59 AND cdbl(sEntryFee) > 0
		...
		' --- Discount Note ????
		sOtherNote="Test discount note -- ADDING ******************************************** THESE"
  	...
  	'sMaxFamMembers=0
  	'TotQualifyingFamMemb=3
		sTEntryFeeFamily=300
		sMaxFamMembers = 3
		sTEntryFeeFamExtra = 150
...
' ---------------------------------------------------------------------------------------
   SUB Write_Waiver_ToFolder (file_prefix, html_var, sTourID, sMemberID, folder_name)
' ---------------------------------------------------------------------------------------
...
ebody = ebody & "<TABLE BORDER=4 ALIGN=CENTER CELLPADDING=3 CELLSPACING=0 BGCOLOR="&TableColor1&" width=320px >"
ebody = ebody & "<TR>"
...
' HQWaiverEmail="rankingserrors@usawaterski.org"
...
' --------------------------------------------------------------------------------------- 
  SUB SendEmailFromGenericMethod (eMailTo,eMailCC,eMailBCC,eMailFrom,eMailSubj,eMailBody)
' --------------------------------------------------------------------------------------- 
...
' ----------------------------------------------------------------------------------------------------------------
  SUB SendEmailFromGenericMethodAndReplyTo (eMailTo,eMailCC,eMailBCC,eMailFrom,eMailReplyTo, eMailSubj,eMailBody)
' ---------------------------------------------------------------------------------------------------------------- 
...
objMessage.HTMLBody = eMailBody
 ...
 ' -- Validation in SQL
'  LEN(Email)-LEN(REPLACE(Email,'@',''))=1 AND CHARINDEX('..',Email)=0 AND CHARINDEX('.',Email)>0 AND CHARINDEX(')',Email)=0 AND CHARINDEX('/',Email)=0 AND CHARINDEX(':',Email)=0 AND CHARINDEX(' ',Email)=0
 
' --- Finally send the message, and then clear that object ---
IF TRIM(eMailTo)<>"" AND InStr(eMailTo,"@")>0 AND InStr(eMailTo,".")>0 AND InStr(eMailTo,"..")=0 AND InStr(eMailTo,"/")=0 AND InStr(eMailTo,":")=0 AND InStr(TRIM(eMailTo)," ")=0 THEN
' IF TRIM(eMailTo)<>"" AND LEN(TRIM(eMailTo))>=10 AND InStr(eMailTo,"@")>0 AND InStr(eMailTo,".")>0 THEN
		objMessage.Send
END IF
set objMessage = Nothing
...
IF AdminMenuLevel>50 THEN 
		response.write("<br> sMemberID = " & sMemberID)
		response.write("<br> sTDateS = " & sTDateS)
END IF

sMembAge = AgeAtDate_New(sTDateS, sMemberID)		
' --- Function finds Member Age
...
IF TestExpireDate=1 AND (sMemberID="000001151" OR sMemberID="1151") AND DateDiff("d", sEffectiveto, sTDateE) < 0 THEN
		response.write("<br> - Line 2121 IF")
		Session("sExpirationStatusText")="OK - "&sEffectiveto	
		Session("sExpirationStatusColor")="blue"
ELSEIF TestExpireDate=1 AND (sMemberID="000001151" OR sMemberID="1151") AND DateDiff("d", sEffectiveto, sTDateE) > 0 THEN	
...
'IF (sMemberID="000001151" OR sMemberID="1151") AND TestMode="yes"  THEN
