...
ThisFileName="/rankings/CCReg2019_TONY.asp"
...
TestMode = false	' --- Testing of original CC method when mode is set to true ---
...
ReturnOnSuccess="Test_PaymentLaunch.asp"	' --- Defines redirection when transaction is a success OBSOLETE? ---
...
' www.usawaterski.org/rankings/CCReg2016_Test.asp?sLP=Test&Action=new&sMemberID=000001151&sTourID=18M999A&sOrderNo=123&CCAmount=32.12
...
<script type="text/javascript">
	...
	function verifyandcontinue() {	
		...
		var CCCardNumber = document.getElementById('CCCardNumber').value;
	...
	function validatedatalength(fieldname) {
	...
	function disableformfields() {											
		...
		document.getElementById("CCFirstName").disabled = true;
		document.getElementById("CCLastName").disabled = true;
		document.getElementById("CCAddress1").disabled = true;
		document.getElementById("CCCity").disabled = true;
		document.getElementById("CCState").disabled = true;
		document.getElementById("CCZipCode").disabled = true;
		document.getElementById("CCPhone").disabled = true;
		document.getElementById("CCEmail").disabled = true;
		document.getElementById("CCCardNumber").disabled = true;
		document.getElementById("CCV").disabled = true;
		document.getElementById("CCExpMonth").disabled = true;
		document.getElementById("CCExpYear").disabled = true;
	...
	function enableformfields() {
		...
		// -- Enable the fields if addditional editing is required --
		document.getElementById("CCFirstName").disabled = false;
		document.getElementById("CCLastName").disabled = false;
		document.getElementById("CCAddress1").disabled = false;
		document.getElementById("CCCity").disabled = false;
		document.getElementById("CCState").disabled = false;
		document.getElementById("CCZipCode").disabled = false;
		document.getElementById("CCPhone").disabled = false;
		...
		document.getElementById("CCExpMonth").disabled = false;
		document.getElementById("CCExpYear").disabled = false;
	}		
...
' -------------------
    ...
    IF Action="Verify & Continue" AND FieldErr=0 THEN %>
      ...
      <tr> 
		...
		<div id="uppertable" style="text-align:center; width:90%">
			<div class="" id="" style="text-align:center; font-size:11pt; font-weight:bold;" >Payment Processing</div>
			<div class="" id="">Complete the information below to make a payment for your event sanctioning.  Payment processing is managed on a secure server.  Your credit card is not stored locally on any USA Water Ski server.  If you are using Internet Explorer, you may obtain a ‘Security Report’ and review information on secure servers by clicking on the Padlock icon located at the top of this page to the right of the URL field.</div>		
    ...
    <TABLE ALIGN="center" class="innertable" width=90%>   	
      ...
      <tr> 
				...
				<td align="left">
					<a title="First Name must match name on the credit card">
						<input type="text" <% =CCFormStatus %> id="CCFirstName" name="CCFirstName" value= "<% =CCFirstName %>" MaxLength=20 size="24">
					</a>
					<div id="FirstNameError" style="font-size:3; color:red; display:none;">*</div>
				</td>
        <td align=right>
      ...
      <tr>
				...
				<td align=left colspan=3>
					<a title="Last Name must match name on the credit card">
						<input type="text" <% =CCFormStatus %> id="CCLastName" name="CCLastName" value= "<% =CCLastName %>" MaxLength=25 size="29">
					</a>
					<div id="LastNameError" style="font-size:3; color:red; display:none;">*</div>
				</td>
      </tr>
      <tr>
				<td ALIGN="right">
				...
				<td colspan=3>
					<input type="text" <% =CCFormStatus %> id="CCAddress1" name="CCAddress1" value= "<% =CCAddress1 %>" MaxLength=30 size="30">
					<div id="Address1Error" style="font-size:3; color:red; display:none;">*</div>
				</td>	
      </tr>
      <tr>
				<td ALIGN="right">
				...
				<td align=left>
					<input type="text" <% =CCFormStatus %> id="CCCity" name="CCCity" value= "<% =CCCity %>" MaxLength=20 size="24">
					<div id="CityError" style="font-size:3; color:red; display:none;">*</div>
				</td>
				<td>
					...
					IF Action="Verify & Continue" THEN 
							%>
							<input type="text" <% =CCFormStatus %> id="CCState" name="CCState" value= "<% =CCState %>" size="2">
							<div id="StateError" style="font-size:3; color:red; display:none;">*</div>
							<%

					ELSE 
							StateArray = Split(USStatesList2,",")  
							%>
							<select id="CCState" name="CCState" <% =CCFormStatus %>>
							<%
				...
				<td>	
					...
					<input type="text" <% =CCFormStatus %> id="CCZipCode" name="CCZipCode" value= "<% =CCZipCode %>" MaxLength=6 size="10"><%
      ...
      <tr> 
				...
				<td colspan="1">
					<input type="text" <% =CCFormStatus %> id="CCPhone" name="CCPhone" value= "<% =CCPhone %>" MaxLength=10 size="14"><%
				...
				<td colspan="1">
					<input type="text" <% =CCFormStatus %> id="CCEmail" name="CCEmail" value= "<% =CCEmail %>" MaxLength=35 size="38"><%
   ...
   <TABLE ALIGN="center" class="innertable" width=90% >   	
		...
		<tr>	
			...
			<td colspan=4>
				...
				<input type="text" MAXLENGTH=16 <% =CCFormStatus %> id="CCCardNumber" name="CCCardNumber" value= "<% =CCCardNumber %>" size="16">
				</a><%
			...
			<td colspan=2>
					...
					%>
					<select id="CCExpMonth" name="CCExpMonth" <% =CCFormStatus %>>
						<%
		  		...
		  		NEXT  %>
					...
					<select id="CCExpYear name="CCExpYear" <% =CCFormStatus %>><%
		  		response.write("<option value = 0 >NA</option>")
    ...
    <tr> 
			...
			<td ALIGN="left" colspan=4>
				<a title="Security Code is a 3 or 4 digit number located on the rear of your credit card">
					<input type="text" <% =CCFormStatus %> id="CCV" name="CCV" value= "<% =CCV %>" MaxLength=4 size="5">
				</a><%
...
' ---------------------------
	...
	SendString = SendString & "x_delim_data=false&"
