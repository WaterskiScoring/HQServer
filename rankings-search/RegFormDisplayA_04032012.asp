...
'response.write("<br>sGREntryFee3 = "&sGREntryFee3)
	...
	<tr><%
		...
		ELSE %>
			<TD ALIGN="right"><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>>1st Ride&nbsp;</FONT></TD><%
		END IF 
	...
	<tr><%
		IF sTPandC<>true AND Cdbl(sTEntryFee2)>cdbl(0.00) THEN %> 	  
			<TD ALIGN="right"><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>>2 Events &nbsp;</FONT></TD><%
		ELSEIF Cdbl(sTEntryFee2)>0.00 THEN %>
			<TD ALIGN="right"><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>>2nd Ride&nbsp;</FONT></TD><%
		ELSE  %>
			<TD>&nbsp;</td><%		
		END IF 
...
'IF MarkTester THEN response.write("<br>sTEntryFee2="& sTEntryFee2)
		...
		IF Cdbl(sGREntryFee2)>cdbl(0.00) THEN %>		
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sGREntryFee2,2) %></FONT></TD><%
		...
		IF sRSurchg>0 AND Cdbl(sTEntryFee2)>cdbl(0.00) THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sTEntryFee2+sRSurchg,2) %></FONT></TD><%		
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF %>
	...
	<tr><%
		IF sTPandC<>true AND Cdbl(sTEntryFee3)>0.00 THEN %> 	  
			<TD ALIGN="right"><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>>3+ Events&nbsp;</FONT></TD><%
		ELSEIF Cdbl(sTEntryFee3)>0.00 THEN %>
			<TD ALIGN="right"><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>>3+ Per Ride&nbsp;</FONT></TD><%
		ELSE  %>
			<TD>&nbsp;</td><%		
		END IF 

		IF Cdbl(sGREntryFee3)>0.00 THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sGREntryFee3,2) %></FONT></TD><%		
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF
		...
		IF sRSurchg>0 AND Cdbl(sTEntryFee3)>cdbl(0.00) THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sTEntryFee3+sRSurchg,2) %></FONT></TD><%		
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF %>
	...
	<tr><%
		...
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF 
		IF sGrassOffered=true AND sTEntryFeeFamily>cdbl(0.00) THEN %>
	...
	<tr><%
		...
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF 
		IF sGrassOffered=true AND sTEntryFeeFamExtra>cdbl(0.00) THEN %>
		...
		IF sRSurchg>0 AND sTEntryFeeFamExtra>cdbl(0.00) THEN %>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<% =fontsize2 %> >&nbsp;<% =FormatCurrency(sTEntryFeeFamExtra,2) %></FONT></TD><%
		ELSE %>
			<TD ALIGN="center">&nbsp;</td><%
		END IF %>
	...
	<tr><%
		IF TRIM(sOF1Desc)<>"" THEN %> 	  
			<td>&nbsp;</td>
			<TD ALIGN="right" Colspan=4><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>><%=sOF1Desc%> &nbsp;</FONT></TD>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>><% IF sOF1Required=true THEN response.write("Yes") ELSE response.write("No") END IF %> </FONT></TD>
			<TD ALIGN="left"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>> &nbsp;<%=FormatCurrency(sOF1Amt,2)%></FONT></TD><%
		ELSE %>
			<td>&nbsp;</td>
			<TD Colspan=4>&nbsp;</td>
			<td>&nbsp;</td>
	...
	<tr><%
		IF TRIM(sOF2Desc)<>"" THEN %> 	  
			<td>&nbsp;</td>
			<TD ALIGN="right" Colspan=4><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>><%=sOF2Desc%> &nbsp;</FONT></TD>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>><% IF sOF2Required=true THEN response.write("Yes") ELSE response.write("No") END IF %> </FONT></TD>
			<TD ALIGN="left"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>> &nbsp;<%=FormatCurrency(sOF2Amt,2)%></FONT></TD><%
		ELSE %>
			<td>&nbsp;</td>
			<TD Colspan=4>&nbsp;</td>
			<td>&nbsp;</td>
	...
	<tr><%
		IF TRIM(sOF3Desc)<>"" THEN %> 	  
			<td>&nbsp;</td>
			<TD ALIGN="right" Colspan=4><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>><%=sOF3Desc%> &nbsp;</FONT></TD>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>><% IF sOF3Required=true THEN response.write("Yes") ELSE response.write("No") END IF %> </FONT></TD>
			<TD ALIGN="left"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>> &nbsp;<%=FormatCurrency(sOF3Amt,2)%></FONT></TD><%
		ELSE %>
			<td>&nbsp;</td>
			<TD Colspan=4>&nbsp;</td>
			<td>&nbsp;</td>
	...
	<tr><%
		IF TRIM(sOF4Desc)<>"" THEN %> 	  
			<td>&nbsp;</td>
			<TD ALIGN="right" Colspan=4><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>><%=sOF4Desc%> &nbsp;</FONT></TD>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>><% IF sOF4Required=true THEN response.write("Yes") ELSE response.write("No") END IF %> </FONT></TD>
			<TD ALIGN="left"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>> &nbsp;<%=FormatCurrency(sOF4Amt,2)%></FONT></TD><%
		ELSE %>
			<td>&nbsp;</td>
			<TD Colspan=4>&nbsp;</td>
			<td>&nbsp;</td>
	...
	<tr><%
		IF TRIM(sOF5Desc)<>"" THEN %> 	  
			<td>&nbsp;</td>
			<TD ALIGN="right" Colspan=4><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>><%=sOF5Desc%> &nbsp;</FONT></TD>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>><% IF sOF5Required=true THEN response.write("Yes") ELSE response.write("No") END IF %> </FONT></TD>
			<TD ALIGN="left"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>> &nbsp;<%=FormatCurrency(sOF5Amt,2)%></FONT></TD><%
		ELSE %>
			<td>&nbsp;</td>
			<TD Colspan=4>&nbsp;</td>
			<td>&nbsp;</td>
	...
	<tr><%
		IF TRIM(sOF6Desc)<>"" THEN %> 	  
			<td>&nbsp;</td>
			<TD ALIGN="right" Colspan=4><FONT COlOR="<%=TextColor1%>" size=<%=fontsize2%>><%=sOF6Desc%> &nbsp;</FONT></TD>
			<TD ALIGN="center"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>><% IF sOF6Required=true THEN response.write("Yes") ELSE response.write("No") END IF %> </FONT></TD>
			<TD ALIGN="left"><FONT COlOR="<%=TextColor2%>" size=<%=fontsize2%>> &nbsp;<%=FormatCurrency(sOF6Amt,2)%></FONT></TD><%
		ELSE %>
			<td>&nbsp;</td>
			<TD Colspan=4>&nbsp;</td>
			<td>&nbsp;</td>
...
<div class="<% IF nav=2 THEN response.write("accordionHeaderSelected") ELSE response.write("accordionHeader") END IF %>">
  ...
  <div class="innertable" style="display:<% IF nav=2 THEN response.write("block") ELSE response.write("none") END IF %>;">
	...
	IF LEFT(Session("sMembCanSkiText"),2)<>"OK" OR LEFT(Session("sExpirationStatusText"),2)<>"OK" THEN  
			...
			<tr>
		  	<rd ALIGN="center" colspan=4><font size="3" color="red"><b>IMPORTANT NOTICE:</b></FONT></TD>
			</tr>
			<tr>
    	...
    	<form name="VerifyUpgradeForm" method="post" action="/rankings/<%=RegFileName%>?sRunByWhat=VerifyUpgrade&nav=2" id="VerifyUpgradeForm">
		  	<td width=50% align="center" colspan=2><input type="submit" name="Verify Renewal" value="Verify <%=UpgradeText%>" style="width:15em" title="Use this link to cause the entry form to verify that your renewal or upgrade is complete. "></td>
			</form>
	  		...
	  		<TABLE>
	  			<TR>
						<td width=25% align=left >
	  					<font color="white" size="<%=fontsize2%>" face="<%=font1%>"><b>STEP 3 - Entry Form </b></font>
	  				</td>
	  			</TR>
	  		</TABLE>
	  		<%
		  ...
		  IF TRIM(sTEvent(EvtNo))<>"" THEN  %>
			...
			IF sShowGR(EvtNo)=true THEN %>
				<td align="center" width="<%=sClassWidth%>px">
					<input type=radio NAME="<%=fFeeClass%>" VALUE="G" <% IF sFeeClass(EvtNo)="G" THEN response.write("checked "&AllObjectStatus) %>>
				</td><%
			ELSE %>
				<td>&nbsp;</td><%
			END IF
			...
			IF sShowRec(EvtNo)=true AND TRIM(Session("sEnableRec"))="Y" THEN %>
					<td align="center" width="<%=sClassWidth%>px">
						<input type=radio NAME="<%=fFeeClass%>" VALUE="R" <% IF sFeeClass(EvtNo)="R" THEN response.write("checked "&AllObjectStatus) %>>
					</td><%
    ...
    <div id="RegPanel4">	 
      ...
      <form name="FinancialForm" method="post" action="/rankings/<%=RegFileName%>" id="FinancialForm">	
      	...
      	<table class="spacetable" width="100%">
		  		...
		  		<td align=left width=100px><input type="text" name="sMembRegDate" value="<% =sMembRegDate %>" MAXLENGTH=10 size="10" <% =MembRegDateStatus %>></td>
				</tr>
				<%
...
'response.write("<br>sMembAge="&sMembAge)
		...
		IF sOffDiscPerc <> cdbl(0) THEN %>
			<tr>	
			  <td colspan="4">
			    ...
			    <font size=<% =fontsize2 %> >&nbsp;&nbsp;Check if you are a rated official willing to work the tournament.</font>
			  </td><%  
	  ...
	  ' ----------------------------
		...
		IF TRIM(sOF1Desc)<>"" THEN %>
		  <tr>
		    <td colspan=4 style="height:2em;">
					<font color="<%=textcolor1%>" size=<% =fontsize2 %>>&nbsp;&nbsp;&nbsp;&nbsp;SPECIAL: <%=sOF1Desc%> at <%=FormatCurrency(sOF1Amt)%>.</font>
					&nbsp;&nbsp;
		    <%'/td
		    ...
		    'td align=left
				...
				' AllObjectStatus ="disabled"
				LoadValuePulldown "sOF1Qty", sOF1Qty, 0, sOF1MaxQty, 1, AllObjectStatus, "true" 
				%>
		    </td>
		    <td align="right"><font color="<%=textcolor1%>" size=<% =fontsize2 %>>Cost</font></td>
		    <td align="right"><font color="<%=textcolor2%>" size=<% =fontsize2 %>><%=FormatCurrency(sOF1Fee)%>&nbsp;&nbsp;</font></td>
		  </tr><%
		END IF
		...
		IF TRIM(sOF2Desc)<>"" THEN %>
		  <tr>
			...
			<font color="<%=textcolor1%>" size=<% =fontsize2 %>>&nbsp;&nbsp;&nbsp;&nbsp;<%=sOF2Desc%> at <%=FormatCurrency(sOF2Amt)%>.</font>
		    </td>
		    <td align=left><%

			'AllObjectStatus ="disabled"
			LoadValuePulldown "sOF2Qty", sOF2Qty, 0, sOF2MaxQty, 1, AllObjectStatus, "true" %>
		    </td>
		    <td align="right"><font color="<%=textcolor1%>" size=<% =fontsize2 %>>Cost</font></td>
		    <td align="right"><font color="<%=textcolor2%>" size=<% =fontsize2 %>><%=FormatCurrency(sOF2Fee)%>&nbsp;&nbsp;</font></td>
		  </tr><%
		END IF
		...
		IF TRIM(sOF3Desc)<>"" THEN %>
		  <tr>
		    <td colspan=3 style="height:2em;">
			<font color="<%=textcolor1%>" size=<% =fontsize2 %>>&nbsp;&nbsp;&nbsp;&nbsp;<%=sOF3Desc%> at <%=FormatCurrency(sOF3Amt)%>.</font>
		    </td>
		    <td align=left><%
			'AllObjectStatus ="disabled"
			LoadValuePulldown "sOF3Qty", sOF3Qty, 0, sOF3MaxQty, 1, AllObjectStatus, "true" %>
		    </td>
		    <td align="right"><font color="<%=textcolor1%>" size=<% =fontsize2 %>>Cost</font></td>
		    <td align="right"><font color="<%=textcolor2%>" size=<% =fontsize2 %>><%=FormatCurrency(sOF3Fee)%>&nbsp;&nbsp;</font></td>
		  </tr><%
		END IF
		...
		IF TRIM(sOF4Desc)<>"" THEN %>
		  <tr>
		    <td colspan=3 style="height:2em;">
			<font color="<%=textcolor1%>" size=<% =fontsize2 %>>&nbsp;&nbsp;&nbsp;&nbsp;<%=sOF4Desc%> at <%=FormatCurrency(sOF4Amt)%>.</font>
		    </td>
		    <td align=left><%

			'AllObjectStatus ="disabled"
			LoadValuePulldown "sOF4Qty", sOF4Qty, 0, sOF4MaxQty, 1, AllObjectStatus, "true" %>
		    </td>
		    <td align="right"><font color="<%=textcolor1%>" size=<% =fontsize2 %>>Cost</font></td>
		    <td align="right"><font color="<%=textcolor2%>" size=<% =fontsize2 %>><%=FormatCurrency(sOF4Fee)%>&nbsp;&nbsp;</font></td>
		  </tr><%
		END IF
		...
		IF TRIM(sOF5Desc)<>"" THEN %>
		  <tr>
		    <td colspan=3 style="height:2em;">
			<font color="<%=textcolor1%>" size=<% =fontsize2 %>>&nbsp;&nbsp;&nbsp;&nbsp;<%=sOF5Desc%> at <%=FormatCurrency(sOF5Amt)%>.</font>
		    </td>
		    <td align=left><%

			'AllObjectStatus ="disabled"
			LoadValuePulldown "sOF5Qty", sOF5Qty, 0, sOF5MaxQty, 1, AllObjectStatus, "true" %>
		    </td>
		    <td align="right"><font color="<%=textcolor1%>" size=<% =fontsize2 %>>Cost</font></td>
		    <td align="right"><font color="<%=textcolor2%>" size=<% =fontsize2 %>><%=FormatCurrency(sOF5Fee)%>&nbsp;&nbsp;</font></td>
		  </tr><%
		END IF
		...
		IF TRIM(sOF6Desc)<>"" THEN %>
		  <tr>
		    <td colspan=3 style="height:2em;">
			<font color="<%=textcolor1%>" size=<% =fontsize2 %>>&nbsp;&nbsp;&nbsp;&nbsp;<%=sOF6Desc%> at <%=FormatCurrency(sOF6Amt)%>.</font>
		    </td>
		    <td align=left><%

			'AllObjectStatus ="disabled"
			LoadValuePulldown "sOF6Qty", sOF6Qty, 0, sOF6MaxQty, 1, AllObjectStatus, "true" %>
		    </td>
		    <td align="right"><font color="<%=textcolor1%>" size=<% =fontsize2 %>>Cost</font></td>
		    <td align="right"><font color="<%=textcolor2%>" size=<% =fontsize2 %>><%=FormatCurrency(sOF6Fee)%>&nbsp;&nbsp;</font></td>
		  </tr><%
		END IF
...
'END IF
		...
		IF MainStatusValue="Verify" THEN %>
		     <tr>
...
RegFeeAmount=sTotalFormFees-cdbl(sTotalPreviousPayments)-sBanquetTot-sAWSEFDonation+sBanquetTotTrans+sAWSEFDonationTrans-sOF1Fee+sOF1FeeTrans-sOF2Fee+sOF2FeeTrans-sOF3Fee+sOF3FeeTrans-sOF4Fee+sOF4FeeTrans-sOF5Fee+sOF5FeeTrans-sOF6Fee+sOF6FeeTrans
...
ItemNo=0
IF RegFeeAmount<0 OR sBanquetTot-sBanquetTotTrans<0 OR sAWSEFDonation-sAWSEFDonationTrans<0 OR sOF1Fee-sOF1FeeTrans<0 OR sOF2Fee-sOF2FeeTrans<0 OR sOF3Fee-sOF3FeeTrans<0 OR sOF4Fee-sOF4FeeTrans<0 OR sOF5Fee-sOF5FeeTrans<0 OR sOF6Fee-sOF6FeeTrans<0 THEN
	ItemNo=ItemNo+1
...
'response.end
	...
	IF ThisInvAmt > 0 THEN %>
		...
		IF sPayType="Card" THEN 	' --- Send to HQ CC processor --- %>
			  ...
			  <table align="center" width=90%>
			    <tr>
			      <form action="/rankings/<%=CardFileName%>?action=new&CCAmount=<%=ThisInvAmt%>&sOrderNo=<%=sOrderNo%>" method=post>
	...
	<TABLE ALIGN="Center" class="innertable" width=80% >
	  ...
	  IF TestValidAdminCode=true OR adminmenulevel >= 20 THEN %>
		...
		<form name="NewEntryForm" method="post" action="/rankings/<%=RegFileName%>?sRunByWhat=Tour" id="NewEntryForm">
		     <input type="submit" name="NewTour" value="New Entry" style="<%=SpecialUserButtonStyle%>" title="Create another online entry" <%=MainButtonStatus%>>
		</form>
	    </td>
...
' -----------------------
	...
	' --- SPECIAL WAIVER code - if values are set then additional pages display in Waiver section ---

	IF LEFT(sTourID,6)="12S999" OR LEFT(sTourID,6)="11M999" THEN
	'IF LEFT(sTourID,6)="12S999" THEN
	...
	' - CASE 4 - Thank you which triggers save
