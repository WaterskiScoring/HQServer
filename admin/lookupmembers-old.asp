...
'		Set tempFSO=nothing
	...
	ELSE
		...
		NumMems = 0
		    ...
		    DO WHILE NOT RS.EOF 
				...
				%><tr>
           		...
           		<TD ALIGN="Center" vAlign="top" BGCOLOR="#FFFFFF"><FONT COlOR="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><a href="DisplayOneMember.asp?MemberID=<%=rs("memberid")%>"><%=rs("LastName")%>,&nbsp;<%=rs("FirstName")%></a></FONT></TD>
		...
		IF NumMems = 1 then
			  rs.close
			  set rs=nothing
				response.redirect "DisplayOneMember.asp?MemberID=" & LastMemID
		END IF
