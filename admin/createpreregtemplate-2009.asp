...
' First we establish if there are qualifications for this TourID

sSQL = "Select count(*) as QfyNum From Cobra00025.USAWSRank.RegisterQualify"
...
sSQL2 = sSQL2 & " Left Join	(Select MemberID, Div, Reg_Ski, AWSA_Rat as SlmRat,"
...
sSQL2 = sSQL2 & " Left Join (Select MemberID, Weight, BibNo, 'YES' as PreReg,"
sSQL2 = sSQL2 & " Case when Len(RampHeight) < 3 then RampHeight else"
sSQL2 = sSQL2 & " left(RampHeight,1) + right(RampHeight,1) end as JRamp"
sSQL2 = sSQL2 & " From Cobra00025.USAWSRank.RegisterGenNew" 
sSQL2 = sSQL2 & " Where left(TourID,6) = '" & left(Session("TournamentID"),6)
sSQL2 = sSQL2 & "') as PR on MX.MemberID = PR.MemberID"

sSQL3 = " Left Join (Select MemberID, Div as SDiv, CASE when FeeClass='G'"
...
' sSQL3 = sSQL3 & " CASE when SkiedRegls = ' ' then ' '"
sSQL3 = sSQL3 & " CASE when QfyStatus = 'Qualified' then 'Y' else ' ' end as SQfy"
sSQL3 = sSQL3 & " From Cobra00025.USAWSRank.RegisterQualify Where Left(Event,1) = 'S'" 
sSQL3 = sSQL3 & " and left(TourID,6) = '" & left(Session("TournamentID"),6)
...
sSQL3 = sSQL3 & " From Cobra00025.USAWSRank.RegisterQualify Where Left(Event,1) = 'T'" 
sSQL3 = sSQL3 & " and left(TourID,6) = '" & left(Session("TournamentID"),6)
...
sSQL3 = sSQL3 & " From Cobra00025.USAWSRank.RegisterQualify Where Left(Event,1) = 'J'" 
sSQL3 = sSQL3 & " and left(TourID,6) = '" & left(Session("TournamentID"),6)
...
'			Otherwise insert Ranking Levels by Events.
			...
			objExcelPreReg.Fields(22).Value = SPaid
			objExcelPreReg.Fields(23).Value = TPaid
			objExcelPreReg.Fields(24).Value = JPaid

			objExcelPreReg.Fields(25).Value = "Pre-Registered"
	...
	ELSEIF objRS("EffTo") >= cdate(session("TournamentDate")) and objRS("CanSki") = True and objRS("Waiver") > 0 THEN
	    ...
	    objExcelActive.Fields(22).Value = "Yes"
	...
	ELSE
		...
		objExcelInActive.Fields(22).Value = "    No"
		...
		IF objRS("EffTo") < cdate(session("TournamentDate")) THEN 
			IF objRS("CanSki") = False THEN
				objExcelInActive.Fields(23).Value = "Nds Rnw/Upg" 
				objExcelInActive.Fields(24).Value = FormatNumber(MemPrice(MT)+MemUpgrd(MT),2)
			ELSE
				objExcelInActive.Fields(23).Value = "Needs Renew" 
				objExcelInActive.Fields(24).Value = FormatNumber(MemPrice(MT),2)
			END IF
		ELSE 
			IF objRS("CanSki") = False THEN
				objExcelInActive.Fields(23).Value = "Needs Upgrd" 
				objExcelInActive.Fields(24).Value = FormatNumber(MemUpgrd(MT),2)
			ELSE
				objExcelInActive.Fields(23).Value = "Nds Ann Wvr" 
				objExcelInActive.Fields(24).Value = FormatNumber(0,2)
			END IF				
		END IF
