...
Dim sMemberID, sPhoneNumber, sZipcode, sEMailID
Dim HQPhoneNumber, HQZipcode, HQEMailID

sMemberID = Request("MemberID")

if sMemberID = 0 or len(sMemberID) = 0 THEN response.redirect "FindMember.asp?FormStatus=newsearch"

sPhoneNumber = Request("PhoneNumber")
sZipcode = Request("Zipcode")
sEMailID = Request("EMailID")

sSQL = "SELECT PT.[Last Name] as LastName, PT.[First Name] as FirstName," 
sSQL = sSQL & " PT.[Company Name] as CompanyName, PA.Phone as PhoneNumber,"
sSQL = sSQL & " PT.EMail, PT.Password, PA.Address1, PA.Address2,"
sSQL = sSQL & " PA.City, PA.State, PA.Zip as Zipcode"
...
HQPhoneNumber = RS("PhoneNumber")
HQZipcode = RS("Zipcode")
HQEMailID = RS("Email")

IF mid(HQZipcode,6,1) = "-" AND (len(trim(HQPhoneNumber)) <= 6 or instr(HQEMailID,"@")) THEN HQZipcode = left(HQZipcode,5)
...
<head>
<title>Validate Club Member</title>
  ...
  <tr> 
    <td bgcolor="#42639F">
      ...
      <p align="center"><font face="Verdana" size="6" color="#FFFFFF">
      	USA Water Ski Club Validation</font></p>
      <p align="center"><font face="Verdana" size="3" color="#FFFFFF"><b>
      	Validation for:&nbsp;&nbsp;&nbsp;&nbsp; <%=RS("CompanyName")%>&nbsp;&nbsp;&nbsp;&nbsp;
      	<%=RS("City")%>, <%=RS("State")%>&nbsp;&nbsp;&nbsp;&nbsp; ( <%=RS("LastName")%>, <%=RS("FirstName")%> )</font></b></p>
      <p>&nbsp;</p>
    </td>
  </tr>
</table>  
...
IF len(sPhoneNumber) > 0 THEN NumInput = NumInput + 1
IF len(sZipcode) > 0 THEN NumInput = NumInput + 1
IF len(sEMailID) > 0 THEN NumInput = NumInput + 1
	
IF NumInput > 0 THEN

	IF len(trim(HQPhoneNumber)) > 6 THEN
		IF right(trim(sPhoneNumber),4) <> right(trim(HQPhoneNumber),4) THEN
			ErrFlds = "PhoneNumber"
		END IF
	END IF
...
<% ELSE %>
	...
	<form action="ValidateClub.asp" method="post">
	...
	indeed <b><i>are</i></b> the club contact you selected, you will need to supply the 
	following details.&nbsp; What you enter here will be checked against your club's 
	membership record, to ensure security.&nbsp; For each of the items below, please 
	enter the requested information, then click the "Validate" button at the bottom 
	of the form.</p>
		
	<% IF len(trim(HQPhoneNumber)) > 6 THEN %>

		<p>I am showing you the leftmost part of the club contact person's phone number 
		that we have in your club's membership record, as a reminder.&nbsp; Please supply 
		only the final rightmost 4 digits of that phone number as we have it.</p>
	...
	IF iat > 0 THEN %>
		...
		the club's membership record, as a reminder.&nbsp; Please supply only the ID part
		of that E-Mail address -- just the characters to the left of the @ sign -- as we 
		have it on file currently.&nbsp; Don't worry if your E-Mail address has changed	
          ...
          <TR>
            ...
            <TD ALIGN="Left" vAlign="top"><Center><FONT COlOR="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif">Enter Value Here</FONT></Center></TD>
				...
				<% IF len(trim(HQPhoneNumber)) > 6 THEN 
							...
							PhoneHint = "( " & left(trim(HQPhoneNumber),len(trim(HQPhoneNumber))-4) & "???? )" %>

          <TR>
            <TD ALIGN="Left" vAlign="center"><Center><FONT COlOR="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">Phone Number</FONT></Center></TD>
            <TD ALIGN="Left" vAlign="center"><Center><FONT COlOR="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif"><%=PhoneHint%></FONT></Center></TD>
            <TD ALIGN="Left" vAlign="center"><Center><FONT COlOR="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif"><input type="text" name="PhoneNumber" value="<%=sPhoneNumber%>" maxlength=10 size=12></input></FONT></Center></TD>
          </TR>
