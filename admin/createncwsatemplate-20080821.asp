...
sSQL = sSQL & " Case when RD.Team > ' ' Then RD.Team else 'ZZZ' end as Team,"
...
sSQL = sSQL & " from Cobra00025.USAWSRank.Rankings"
sSQL = sSQL & " where RankScore is not Null and Left(Div,1) = 'C'"
sSQL = sSQL & " group by MemberID) as RD on RD.MemberID = MX.MemberID"

sSQL = sSQL & " Order By Case when RD.Team > ' ' Then RD.Team else 'ZZZ' end,"
sSQL = sSQL & " MX.LastName, MX.FirstName, RD.MemberID"
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
''' Now we finally loop through the Extract of Members, merging in the
...
DO until objRS.EOF
	...
	DO WHILE LocalTeamID < "ZZZ" AND LocalTeamID <= trim(objRS("Team"))
	...
	IF objRS("EffTo") >= cdate(session("TournamentDate")) and objRS("CanSki") = True THEN

		IF objRS("Div") = "CM" THEN

			IF (trim(objRS("Team")) <> "ZZZ" AND trim(objRS("Team")) <> LocalMenTeam) OR _
				 (trim(objRS("Team")) = "ZZZ" AND LocalMenTeam <> "ZZZ") THEN
				objExcelAMen.addnew
				objExcelAMen.Fields(0).Value = " "
				objExcelAMen.Update
				LocalMenTeam = trim(ObjRS("Team"))
			END IF
			...
			objExcelAMen.Fields(8).Value = objRS("OffRat")

			objExcelAMen.Fields(16).Value = "Yes"
	
			objExcelAMen.Update

		ELSE

			IF (trim(objRS("Team")) <> "ZZZ" AND trim(objRS("Team")) <> LocalWomTeam) OR _
				 (trim(objRS("Team")) = "ZZZ" AND LocalWomTeam <> "ZZZ") THEN
				objExcelAWomen.addnew
				objExcelAWomen.Fields(0).Value = " "
				objExcelAWomen.Update
				LocalWomTeam = trim(ObjRS("Team"))
			END IF
			...
			objExcelAWomen.Fields(8).Value = objRS("OffRat")
			
			objExcelAWomen.Fields(16).Value = "Yes"
	...
	ELSE
		...
		objExcelUpgRnew.Fields(8).Value = objRS("OffRat")

		objExcelUpgRnew.Fields(16).Value = "    No"
...
<table border="0" cellspacing="0" cellpadding="0">  
  ...
  <table>
      <tr> 
         ...
         Registration Template workbook is now complete and ready to download.</font></td>
      </tr>
      ...
      <tr> 
         ...
         download your Registration Template workbook, then select the "Save As" 
