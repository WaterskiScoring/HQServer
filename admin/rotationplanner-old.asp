...
IF Request("FormStatus") = "PickTour" THEN 
	...
	sSQL = sSQL & " WHERE ST.TSanType = 1 AND ST.TStatus in (0,1,2,4,5)"

	IF session("id") = 850 THEN
...
ELSEIF Request("FormStatus") = "SeePlan" THEN 
  ...
  ' Set up query to pull Team Rotation Plan Table for current TeamID and TourID
	...
	sSQL = sSQL & " RP.SlalomEnt, RP.TrickEnt, RP.JumpEnt, RP.WaiverStat"
	sSQL = sSQL & " FROM Cobra00025.USAWSRank.TeamRotations as RP"	
	sSQL = sSQL & " JOIN USAWaterski.dbo.members as Mem"
	sSQL = sSQL & " on Mem.PersonIDWithCheckDigit = RP.MemberID"
...
ELSEIF Request("FormStatus") = "EditPlan" THEN 
	...
	IF Request("TourID") <> Session("TourID") THEN
	  ...
	  IF Kount = 0 THEN
			...
			sSQL = sSQL & " Sex, SlalomEnt, TrickEnt, JumpEnt, DateUpdated, WaiverStat)"
			sSQL = sSQL & " SELECT Team, '" & Session("TourID") & "', RP.MemberID, RP.Sex,"
			sSQL = sSQL & " RP.SlalomEnt, RP.TrickEnt, JumpEnt, GetDate(), 'C'"
			sSQL = sSQL & " FROM Cobra00025.USAWSRank.TeamRotations as RP"
			sSQL = sSQL & " JOIN USAWaterski.dbo.Members as Mem on"
			sSQL = sSQL & " Mem.PersonIDWithCheckDigit = RP.MemberID"
...
ELSEIF Request("FormStatus") = "PlanOK" THEN 
	...
	For Index = 1 to Request("FormRows")
		...
		IF Request("WaiverStat"&Index) = "C" THEN
			...
			objCDO.From = "NCWSA-Online@usawaterski.org"
...
'			objCDO.CC = """Dave Clark"" <awsatechdude@comcast.net>"
	...
	IF Instr(TDirEmail,"@") > 0 or instr(RegistEmail,"@") > 0 THEN
		...
		objCDO.Subject = "Team (" & Session("TeamID") & ") Entry to " & Session("TourID") & " " & Session("TourName")
		...
		IF Instr(CaptEmail,"@") > 0 THEN objCDO.CC = """" & CaptName & """ <" & CaptEmail & ">"
		objCDO.From = "NCWSA-Online@usawaterski.org"
		objCDO.BCC = """Dave Clark"" <awsatechdude@comcast.net>"
		objCDO.HTMLBody = ebody	
...
IF LastPerson <> Session("id") and Session("id") <> 850 THEN
	...
	IF LastPerson > 0 THEN
		...
		IF Instr(PrevEmail,"@") > 0 and instr(CurrEmail,"@") > 0 THEN
			...
			objCDO.Subject = "NCWSA Team (" & Session("TeamID") & ") Administration Change"
			objCDO.To = """" & PrevFirstName & " " & PrevLastName & """ <" & PrevEmail & ">"
			objCDO.CC = """" & CurrFirstName & " " & CurrLastName & """ <" & CurrEmail & ">"
			objCDO.From = "NCWSA-Online@usawaterski.org"
			objCDO.BCC = """Dave Clark"" <awsatechdude@comcast.net>"
			objCDO.HTMLBody = ebody	
