...
objFSO.CopyFile path & "/Templates/NCWSATemplate2011.xls", path & "/template_with_scores.xls" , True
...
sSQL = sSQL & " Case when TE.Team is not null then 'E' else 'Z' end + Case" 
sSQL = sSQL & " when TR.Team is not null then TR.Team else 'zzz' end as Sorter,"

sSQL = sSQL & " Case when TR.DateInactive is not null then 'I' else 'A' end as TeamStat,"

sSQL = sSQL & " Coalesce(TR.Team,'   ') as Team,"
...
sSQL = sSQL & "') as TE on TE.Team = TR.Team"

'	This subquery pulls Rotation Plan information for this Person/Team/TourID
sSQL = sSQL & " left join Cobra00025.USAWSRank.TeamRotations as RP"
sSQL = sSQL & " on RP.TournAppID = '" & left(strTSanction,6) & "'"
sSQL = sSQL & " and RP.MemberID = MT.PersonIDWithCheckDigit and RP.Team = TR.Team"
...
Dim NextTeamID, LocalTeamID, LocalTeamCd, NextRotation
...
DO until objRS.EOF
	...
	IF objRS("Sorter") = "E0FF" or (left(objRS("Sorter"),1) = "E" and objRS("TeamStat") = "A" and (objRS("SlmEnt") <> "  " or objRS("TrkEnt") <> "  " or objRS("JmpEnt") <> "  ")) THEN
		...
		IF objRS("Div") = "CW" and objRS("Sorter") <> "E0FF" THEN
			...
			objExcelBWomen.addnew
			objExcelBWomen.Fields(0).Value = objRS("MemID")
			...
			IF left(objRS("SlmEnt"),1) = "B" THEN
				objExcelBWomen.Fields(8).Value = Right(objRS("SlmEnt"),1)
			ELSEIF objRS("SlmEnt") = "DD" THEN
				objExcelBWomen.Fields(8).Value = NextRotation
			END IF
				
			IF left(objRS("TrkEnt"),1) = "B" THEN
				objExcelBWomen.Fields(9).Value = Right(objRS("TrkEnt"),1)
			ELSEIF objRS("TrkEnt") = "DD" THEN
				objExcelBWomen.Fields(9).Value = NextRotation
			END IF
				
			IF left(objRS("JmpEnt"),1) = "B" THEN
				objExcelBWomen.Fields(10).Value = Right(objRS("JmpEnt"),1)
			ELSEIF objRS("JmpEnt") = "DD" THEN
				objExcelBWomen.Fields(10).Value = NextRotation
			END IF
		...
		ELSEIF objRS("Div") = "CM" or objRS("Sorter") = "E0FF" THEN
			...
			objExcelBMen.addnew
			objExcelBMen.Fields(0).Value = objRS("MemID")
			...
			ELSEIF objRS("SlmEnt") = "DD" THEN
				objExcelBMen.Fields(8).Value = NextRotation
			END IF
				
			IF left(objRS("TrkEnt"),1) = "B" THEN
				objExcelBMen.Fields(9).Value = Right(objRS("TrkEnt"),1)
			ELSEIF objRS("TrkEnt") = "DD" THEN
				objExcelBMen.Fields(9).Value = NextRotation
			END IF
				
			IF left(objRS("JmpEnt"),1) = "B" THEN
				objExcelBMen.Fields(10).Value = Right(objRS("JmpEnt"),1)
			ELSEIF objRS("JmpEnt") = "DD" THEN
				objExcelBMen.Fields(10).Value = NextRotation
			END IF
